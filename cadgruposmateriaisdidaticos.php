<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROGRUPOSMATERIAISDIDATICOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Grupos de Materiais Didáticos");
	...
	$campos[0][0] = "gmd_nome";
	$campos[0][1] = "Nome do Grupo";

	$cad = new MCLASSGrid();

	$args = "";

	$cad->ordemdefault  = "gmd_nome";
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROGRUPOSMATERIAISDIDATICOSEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROGRUPOSMATERIAISDIDATICOSEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Grupo de Material Didático!", "danger");
		} else {
			$gmd_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($gmd_codigo, "gruposmateriaisdidaticos")) {
				$params = array();
				array_push($params, array('name'=>'gmd_pre_codigo','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'gmd_codigo',    'value'=>$gmd_codigo,      'type'=>PDO::PARAM_INT));
				$sql = Utility::geraSQLDELETE("gruposmateriaisdidaticos", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				Utility::setMsgPopup("Grupo de Material Didático Excluído com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirgrupomaterialdidatico&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Grupos de Materiais Didáticos</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Grupos de Materiais Didáticos</legend>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("gmd_codigo", "Código do Grupo", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("gmd_nome", "Nome do Grupo", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->gmd_uuid."&id=".$row->gmd_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center">
		   <?php echo $row->gmd_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->gmd_nome; ?>
		  </td>

		  <td align="center" valign="botton">
			<button class="form_btn_excluir" name="btnexcluir" id="btnexcluir"
			codigo="<?php echo $row->gmd_codigo; ?>"
			nome="<?php echo $row->gmd_nome; ?>"
