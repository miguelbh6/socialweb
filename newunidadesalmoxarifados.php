<?php
	...
	global $PER_CADASTROUNIDADESALMOXARIFADOS;
	if (!$utility->usuarioPermissao($PER_CADASTROUNIDADESALMOXARIFADOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Unidades de Almoxarifados - 1");
	...
	$cad = new MCLASSGrid();
	$cad->arqlis = "cadunidadesalmoxarifados.php";
	$cad->arqedt = "newunidadesalmoxarifados.php";

	function getDados() {
		foreach($_POST as $key =>$val) {
			if (substr($key, 0, 4) == 'ual_') {
				global $$key;
	...
	function validaDados() {
		global $msg, $utility, $ual_nome, $ual_unidade, $ual_codigo;
		$msg = "";

		//Nome
		if (Utility::Vazio($ual_nome)) {
			$msg = "Nome do Unidade de Almoxarifado Inválido";
		}
		//if ((Utility::Vazio($msg)) && (strlen($ual_nome) < 10)) {
		//	$msg = "Nome do Unidade de Almoxarifado Inválido(Poucos caracteres)";
		//}
		if ((Utility::Vazio($msg)) && ($utility->verificaNomeCadastroExiste($ual_nome, $ual_codigo, "unidadesalmoxarifados"))) {
			$msg = "Nome do Unidade de Almoxarifado Já Existe no Cadastro";
		}

		//Unidade
		if (Utility::Vazio($ual_unidade)) {
			$msg = "Unidade de Almoxarifado Inválido";
		}
		return;
	}
	...
	if ($_GET['acao'] == "inserir") {
		global $PER_CADASTROUNIDADESALMOXARIFADOSINSERIR;
		if (!$utility->usuarioPermissao($PER_CADASTROUNIDADESALMOXARIFADOSINSERIR)) {
			Utility::setMsgPopup("Você não tem acesso a inserir Unidade de Almoxarifado!", "danger");
			Utility::redirect($cad->arqlis);
		}
		...
		//Inserir - Salvar
		if ((isset($_POST['ual_nome'])) && ($subacao == "salvar")) {
			getDados();
			validaDados();
			...
			if (Utility::Vazio($msg)) {
				...
				$UltimoCodigo  = $utility->getProximoCodigoTabela("unidadesalmoxarifados");
				...
				$params = array();
				array_push($params, array('name'=>'ual_codigo',      'value'=>$id,              'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'ual_pre_codigo',  'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'ual_uuid',        'value'=>$uuid,            'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'ual_usu_cadastro','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'ual_datacadastro','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'ual_nome',        'value'=>$ual_nome,        'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'ual_unidade',     'value'=>$ual_unidade,     'type'=>PDO::PARAM_STR));

				$sql = Utility::geraSQLINSERT("unidadesalmoxarifados", $params);
	...
	if ($_GET['acao'] == "editar") {
		...
		//Carrega Dados
		$sql = "SELECT c.* FROM unidadesalmoxarifados c
				WHERE c.ual_pre_codigo = :CodigoPrefeitura
				AND   c.ual_codigo     = :id
				AND   c.ual_uuid       = :uuid";
		...
		if ($numrows != 1) {
			$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Unidades de Almoxarifados - 2");
			Utility::redirect("acessonegado.php");
		}
		...
		//Alterar - Salvar
		if ((isset($_POST['ual_nome'])) && ($subacao == "salvar")) {
			global $PER_CADASTROUNIDADESALMOXARIFADOSALTERAR;
			if (!$utility->usuarioPermissao($PER_CADASTROUNIDADESALMOXARIFADOSALTERAR)) {
				Utility::setMsgPopup("Você não tem acesso a alterar Unidade de Almoxarifado!", "danger");
				Utility::redirect($cad->arqlis);
			}
			...
			if (Utility::Vazio($msg)) {
				...
				$params = array();
				array_push($params, array('name'=>'ual_nome',         'value'=>$ual_nome,        'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'ual_unidade',      'value'=>$ual_unidade,     'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'ual_usu_alteracao','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT,'operador'=>'SET'));
				array_push($params, array('name'=>'ual_dataalteracao','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'ual_pre_codigo',   'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));
				array_push($params, array('name'=>'ual_uuid',         'value'=>$uuid,            'type'=>PDO::PARAM_STR,'operador'=>'WHERE'));
				array_push($params, array('name'=>'ual_codigo',       'value'=>$id,              'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));

				$sql = Utility::geraSQLUPDATE("unidadesalmoxarifados", $params);
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Unidades de Almoxarifados<?php echo $textoacao; ?></div>
		<br/>
...
<fieldset style="width:730px;" class="classfieldset1">
   <legend class="classlegend1">Dados do Unidade de Almoxarifado</legend>
...
<table border="0" width="100%" cellspacing="10" cellpadding="0" align="center">
 <tr>
  <td align="left">
    <label class="classlabel1">Código do Unidade de Almoxarifado:&nbsp;</label>
  	<input type="text" maxlength="100" name="ual_codigo" id="ual_codigo" disabled="disabled" class="classinput1" style="width:150px;text-align:right;background-color:#bcbcbc;" value="<?php echo $ual_codigo; ?>">
  </td>
 </tr>

 <tr>
  <td align="left">
    <label class="classlabel1">Nome do Unidade de Almoxarifado:&nbsp;<label class="labelasterisco">*&nbsp;</label></label>
  	<input type="text" maxlength="50" name="ual_nome" id="ual_nome" class="classinput1 inputobrigatorio" style="width:450px" value="<?php echo $ual_nome; ?>">
  </td>
 </tr>
 ...
 <tr>
  <td align="left">
    <label class="classlabel1">Unidade:&nbsp;<label class="labelasterisco">*&nbsp;</label></label>
  	<input type="text" maxlength="10" name="ual_unidade" id="ual_unidade" class="classinput1 inputobrigatorio" style="width:250px" value="<?php echo $ual_unidade; ?>">
  </td>
 </tr>
