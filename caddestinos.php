<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTRODESTINOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Origens/Destinos");
		Utility::redirect("acessonegado.php");
	}

	$utility->SalvaCadastroAcessado(basename($_SERVER['SCRIPT_FILENAME']), "Cadastro de Origens/Destinos");
	...
	$campos[0][0] = "des_nome";
	$campos[0][1] = "Nome da Origem/Destino";

	$cad = new MCLASSGrid();

	$args = "";

	$cad->ordemdefault  = "des_nome";
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $per_cadastrodestinosexcluir;
		if (!$utility->usuarioPermissao($PER_CADASTRODESTINOSEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Origens/Destinos!", "danger");
		} else {
			$des_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($des_codigo, "destinos")) {
				$params = array();
				array_push($params, array('name'=>'des_pre_codigo','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'des_codigo',    'value'=>$des_codigo,      'type'=>PDO::PARAM_INT));
				$sql = Utility::geraSQLDELETE("destinos", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				Utility::setMsgPopup("Origem/Destino Excluído com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirdestino&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Origens/Destinos</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Origens/Destinos</legend>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("des_codigo", "Código da Origem/Destino", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("des_nome", "Nome da Origem/Destino", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->des_uuid."&id=".$row->des_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center">
		   <?php echo $row->des_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->des_nome; ?>
		  </td>

		  <td align="center" valign="botton">
			<button class="form_btn_excluir" name="btnexcluir" id="btnexcluir"
			codigo="<?php echo $row->des_codigo; ?>"
			nome="<?php echo $row->des_nome; ?>"
