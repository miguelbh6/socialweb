<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROORGAOSSOCIAL)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Órgãos Social");
	...
	$campos[0][0] = "oso_nome";
	$campos[0][1] = "Nome do Órgão";

	$cad = new MCLASSGrid();

	$args = "";

	$cad->ordemdefault  = "oso_nome";
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROORGAOSSOCIALEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROORGAOSSOCIALEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Órgão Social!", "danger");
		} else {
			$oso_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($oso_codigo, "orgaossocial")) {
				$params = array();
				array_push($params, array('name'=>'oso_pre_codigo','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'oso_codigo',    'value'=>$oso_codigo,      'type'=>PDO::PARAM_INT));
				$sql = Utility::geraSQLDELETE("orgaossocial", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				Utility::setMsgPopup("Órgão Social Excluído com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirorgaosocial&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Órgãos Social</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Órgãos Social</legend>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("oso_codigo", "Código do Órgão", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("oso_nome", "Nome do Órgão", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->oso_uuid."&id=".$row->oso_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center">
		   <?php echo $row->oso_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->oso_nome; ?>
		  </td>

		  <td align="center" valign="botton">
			<button class="form_btn_excluir" name="btnexcluir" id="btnexcluir"
			codigo="<?php echo $row->oso_codigo; ?>"
			nome="<?php echo $row->oso_nome; ?>"
