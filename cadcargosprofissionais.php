<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROCARGOSPROFISSIONALSOCIAL)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Cargos/Funções");
	...
	$campos[0][0] = "cpr_nome";
	$campos[0][1] = "Nome do Cargos/Funções";

	$cad = new MCLASSGrid();

	$args = "";

	$cad->ordemdefault  = "cpr_nome";
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROCARGOSPROFISSIONALSOCIALEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROCARGOSPROFISSIONALSOCIALEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Cargo/Função de Profissional Social!", "danger");
		} else {
			$cpr_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($cpr_codigo, "cargosprofissionais")) {
				$params = array();
				array_push($params, array('name'=>'cpr_pre_codigo','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'cpr_codigo',    'value'=>$cpr_codigo,      'type'=>PDO::PARAM_INT));
				$sql = Utility::geraSQLDELETE("cargosprofissionais", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				Utility::setMsgPopup("Cargo/Função Excluído com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluircargoprofissional&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Cargos/Funções de Profissional Social</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Cargos/Funções de Profissional Social</legend>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("cpr_codigo", "Código do Cargo/Função", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("cpr_nome", "Nome do Cargo/Função", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->cpr_uuid."&id=".$row->cpr_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center" valign="botton">
			...
			codigo="<?php echo $row->cpr_codigo; ?>"
			nome="<?php echo $row->cpr_nome; ?>"
