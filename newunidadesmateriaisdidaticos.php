<?php
	...
	global $PER_CADASTROUNIDADESMATERIAISDIDATICOS;
	if (!$utility->usuarioPermissao($PER_CADASTROUNIDADESMATERIAISDIDATICOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Unidades de Materiais Didáticos - 1");
	...
	$cad = new MCLASSGrid();
	$cad->arqlis = "cadunidadesmateriaisdidaticos.php";
	$cad->arqedt = "newunidadesmateriaisdidaticos.php";

	function getDados() {
		foreach($_POST as $key =>$val) {
			if (substr($key, 0, 4) == 'umd_') {
				global $$key;
	...
	function validaDados() {
		global $msg, $utility, $umd_nome, $umd_unidade, $umd_codigo;
		$msg = "";

		//Nome
		if (Utility::Vazio($umd_nome)) {
			$msg = "Nome da Unidade de Material Didático Inválido";
		}
		//if ((Utility::Vazio($msg)) && (strlen($umd_nome) < 10)) {
		//	$msg = "Nome da Unidade de Material Didático Inválido(Poucos caracteres)";
		//}
		if ((Utility::Vazio($msg)) && ($utility->verificaNomeCadastroExiste($umd_nome, $umd_codigo, "unidadesmateriaisdidaticos"))) {
			$msg = "Nome da Unidade de Material Didático Já Existe no Cadastro";
		}

		//Unidade
		if (Utility::Vazio($umd_unidade)) {
			$msg = "Unidade de Material Didático Inválido";
		}
		return;
	}
	...
	if ($_GET['acao'] == "inserir") {
		global $PER_CADASTROUNIDADESMATERIAISDIDATICOSINSERIR;
		if (!$utility->usuarioPermissao($PER_CADASTROUNIDADESMATERIAISDIDATICOSINSERIR)) {
			Utility::setMsgPopup("Você não tem acesso a inserir Unidade de Material Didático!", "danger");
			Utility::redirect($cad->arqlis);
		}
		...
		//Inserir - Salvar
		if ((isset($_POST['umd_nome'])) && ($subacao == "salvar")) {
			getDados();
			validaDados();
			...
			if (Utility::Vazio($msg)) {
				...
				$UltimoCodigo  = $utility->getProximoCodigoTabela("unidadesmateriaisdidaticos");
				...
				$params = array();
				array_push($params, array('name'=>'umd_codigo',      'value'=>$id,              'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'umd_pre_codigo',  'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'umd_uuid',        'value'=>$uuid,            'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'umd_usu_cadastro','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'umd_datacadastro','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'umd_nome',        'value'=>$umd_nome,        'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'umd_unidade',     'value'=>$umd_unidade,     'type'=>PDO::PARAM_STR));

				$sql = Utility::geraSQLINSERT("unidadesmateriaisdidaticos", $params);
	...
	if ($_GET['acao'] == "editar") {
		...
		//Carrega Dados
		$sql = "SELECT c.* FROM unidadesmateriaisdidaticos c
				WHERE c.umd_pre_codigo = :CodigoPrefeitura
				AND   c.umd_codigo     = :id
				AND   c.umd_uuid       = :uuid";
		...
		if ($numrows != 1) {
			$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Unidades de Materiais Didáticos - 2");
			Utility::redirect("acessonegado.php");
		}
		...
		//Alterar - Salvar
		if ((isset($_POST['umd_nome'])) && ($subacao == "salvar")) {
			global $PER_CADASTROUNIDADESMATERIAISDIDATICOSALTERAR;
			if (!$utility->usuarioPermissao($PER_CADASTROUNIDADESMATERIAISDIDATICOSALTERAR)) {
				Utility::setMsgPopup("Você não tem acesso a alterar Unidade de Material Didático!", "danger");
				Utility::redirect($cad->arqlis);
			}
			...
			if (Utility::Vazio($msg)) {
				...
				$params = array();
				array_push($params, array('name'=>'umd_nome',         'value'=>$umd_nome,        'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'umd_unidade',      'value'=>$umd_unidade,     'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'umd_usu_alteracao','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT,'operador'=>'SET'));
				array_push($params, array('name'=>'umd_dataalteracao','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'umd_pre_codigo',   'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));
				array_push($params, array('name'=>'umd_uuid',         'value'=>$uuid,            'type'=>PDO::PARAM_STR,'operador'=>'WHERE'));
				array_push($params, array('name'=>'umd_codigo',       'value'=>$id,              'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));

				$sql = Utility::geraSQLUPDATE("unidadesmateriaisdidaticos", $params);
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Unidades de Materiais Didáticos<?php echo $textoacao; ?></div>
		<br/>
...
<fieldset style="width:730px;" class="classfieldset1">
   <legend class="classlegend1">Dados da Unidade de Material Didático</legend>
 ...
 <tr>
  <td align="left">
    <label class="classlabel1">Código da Unidade:&nbsp;</label>
  	<input type="text" maxlength="100" name="umd_codigo" id="umd_codigo" disabled="disabled" class="classinput1" style="width:150px;text-align:right;background-color:#bcbcbc;" value="<?php echo $umd_codigo; ?>">
  </td>
 </tr>
 ...
 <tr>
  <td align="left">
    <label class="classlabel1">Nome da Unidade:&nbsp;<label class="labelasterisco">*&nbsp;</label></label>
  	<input type="text" maxlength="50" name="umd_nome" id="umd_nome" class="classinput1 inputobrigatorio" style="width:450px" value="<?php echo $umd_nome; ?>">
  </td>
 </tr>
 ...
 <tr>
  <td align="left">
    <label class="classlabel1">Unidade:&nbsp;<label class="labelasterisco">*&nbsp;</label></label>
  	<input type="text" maxlength="10" name="umd_unidade" id="umd_unidade" class="classinput1 inputobrigatorio" style="width:250px" value="<?php echo $umd_unidade; ?>">
  </td>
 </tr>
