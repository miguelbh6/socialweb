<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROUNIDADESSOCIAL)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Unidades Social");
		Utility::redirect("acessonegado.php");
	}

	$utility->SalvaCadastroAcessado(basename($_SERVER['SCRIPT_FILENAME']), "Cadastro de Unidades Social");
	...
	$campos[0][0] = "uso_nome";
	$campos[0][1] = "Nome da Unidade Social";

	$cad = new MCLASSGrid();

	$args = "";

	$cad->ordemdefault  = "uso_nome";
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROUNIDADESSOCIALEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROUNIDADESSOCIALEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Unidade Social!", "danger");
		} else {
			$uso_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($uso_codigo, "unidadessocial")) {
				$params = array();
				...
				$sql = Utility::geraSQLDELETE("unidadessocial", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				Utility::setMsgPopup("Unidade Social Excluída com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirunidadesocial&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Unidades Social</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Unidades Social</legend>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("uso_codigo", "Código da Unidade Social", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("uso_nome", "Nome da Unidade Social", $args); ?>
			</th>
			<th width="80px">
			 	<?php echo $cad->geraURLTitulo("uso_bairro", "Bairro da Unidade Social", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("uso_telefone1", "Telefone 1", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("uso_telefone2", "Telefone 2", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->uso_uuid."&id=".$row->uso_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center">
		   <?php echo $row->uso_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->uso_nome; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->uso_bairro; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->uso_telefone1; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->uso_telefone2; ?>
		  </td>

		  <td align="center" valign="botton">
			<button class="form_btn_excluir" name="btnexcluir" id="btnexcluir"
			codigo="<?php echo $row->uso_codigo; ?>"
			nome="<?php echo $row->uso_nome; ?>"
