...
class RelSintetico1EntradaMateriaisDidaticosPDF extends tFPDF {
	...
	private function geraCabecalho() {
		...
		$this->Cell(278,5,'RELATÓRIO SINTÉTICO DE ENTRADAS DE MATERIAIS DIDÁTICOS',0,1,'C');
	...
	function geraRelatorio() {
		...
		if ($_SESSION["rel_mdi_codigo"] > 0) {
			$aux = " AND m.mdi_codigo = ".$_SESSION["rel_mdi_codigo"];
		} else {
			$aux = "";
		}

		$sql = "SELECT m.mdi_codigo, m.mdi_nome FROM materiaisdidaticos m WHERE m.mdi_pre_codigo = $CodigoPrefeitura $aux ORDER BY m.mdi_nome";
		...
		while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
			$entradas = $utility->getTotalEntradasMateriaisDidaticosDataUnidade($row->mdi_codigo, $_SESSION["rel_emd_dataini"], $_SESSION["rel_emd_datafim"], $_SESSION["rel_emd_uso_codigo"]);
			if ($entradas > 0) {
				$ini = $this->GetY() + 1.3;
				$posY = $this->GetY();
				$i++;
