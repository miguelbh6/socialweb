...
class Rel1EntradaGenerosAlimenticiosPDF extends tFPDF {
	...
	private function geraCabecalho() {
		...
		if (!isset($_SESSION["rel_ega_dataini"])) {
			return;
		}
		...
		$this->Cell(278,5,'RELATÓRIO DE ENTRADAS DE GÊNEROS ALIMENTÍCIOS',0,1,'C');
		...
		$this->SetXY(10, $ini);
		$this->SetFont('Arial','B',12);
		$this->Cell(278,5,'PERÍODO: '.$_SESSION["rel_ega_dataini"].' ATÉ '.$_SESSION["rel_ega_datafim"].$aux_gal_nome,0,1,'C');

		$ini += 5;
		$this->SetXY(10, $ini);
		$this->SetFont('Arial','B',12);
		$this->Cell(278,5,'UNIDADE SOCIAL: '.utf8_decode($utility->getNomeCadastro($_SESSION["rel_ega_uso_codigo"], "unidadessocial")),0,1,'C');
	}
	...
	function geraRelatorio() {
		...
		if (!isset($_SESSION["rel_ega_sql"])) {
			return;
		}

		$sql = $_SESSION["rel_ega_sql"];
		...
		while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
			...
			$this->SetXY(10, $ini);
			$this->Cell(20,5,$row->ega_codigo,0,1,'R');

			$this->SetXY(32, $ini);
			$this->Cell(90,5,substr($utility->getNomeCadastro($row->ega_for_codigo, "fornecedores"),0,50),0,1,'L');

			$this->SetXY(124, $ini);
			$this->Cell(64,5,substr($utility->getValorCadastroCampo($row->ega_for_codigo, "fornecedores", "for_endereco"),0,40),0,1,'L');

			$this->SetXY(252, $ini);
			$this->Cell(35,5,Utility::formataData($row->ega_dataentrada),0,1,'C');

			if ($_SESSION["rel_gal_codigo"] > 0) {
				$aux = " AND i.ieg_gal_codigo = ".$_SESSION["rel_gal_codigo"];
			} else {
				$aux = "";
			}

			$sql = "SELECT i.* FROM itensentradasgenerosalimenticios i
					WHERE i.ieg_pre_codigo = :CodigoPrefeitura
					AND   i.ieg_ega_codigo = :ega_codigo
					$aux
					ORDER BY i.ieg_codigo";

			$params = array();
			array_push($params, array('name'=>'CodigoPrefeitura','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
			array_push($params, array('name'=>'ega_codigo',      'value'=>$row->ega_codigo, 'type'=>PDO::PARAM_INT));
			...
			while ($rowitens = $objQryItens->fetch(PDO::FETCH_OBJ)) {
				...
				$this->SetXY(42, $ini);
				$this->Cell(134,5,substr($utility->getNomeCadastro($rowitens->ieg_gal_codigo, "generosalimenticios"),0,80),0,1,'L');

				$this->SetXY(178, $ini);
				$this->Cell(35,5,$rowitens->ieg_qtd,0,1,'R');

				$this->SetXY(217, $ini);
				$this->Cell(33,5,$rowitens->ieg_lote,0,1,'L');

				$this->SetXY(252, $ini);
				$this->Cell(35,5,Utility::formataData($utility->getValidadeGeneroAlimenticioLote($row->ega_uso_codigo, $rowitens->ieg_gal_codigo, $rowitens->ieg_lote)),0,1,'L');
			}
		}
	}
