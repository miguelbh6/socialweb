...
class Rel1SaidaAlmoxarifadosPDF extends tFPDF {
	...
	private function geraCabecalho() {
		...
		$this->Cell(278,5,'RELATÓRIO DE SAÍDAS DE ALMOXARIFADOS',0,1,'C');
		...
		$this->SetXY(10, $ini);
		$this->SetFont('Arial','B',12);
		$this->Cell(278,5,'UNIDADE SOCIAL: '.utf8_decode($utility->getNomeCadastro($_SESSION["rel_sal_uso_codigo"], "unidadessocial")),0,1,'C');
	}
	...
	function geraRelatorio() {
		...
		while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
			...
			$this->SetXY(10, $ini);
			$this->Cell(20,5,$row->sal_codigo,0,1,'R');

			$this->SetXY(32, $ini);
			$this->Cell(90,5,substr($utility->getNomeCadastro($row->sal_prf_codigo, "profissionais"),0,50),0,1,'L');

			//$this->SetXY(124, $ini);
			//$this->Cell(64,5,substr($utility->getValorCadastroCampo($row->sal_prf_codigo, "profissionais", "prf_endereco"),0,40),0,1,'L');

			//$this->SetXY(190, $ini);
			//$this->Cell(60,5,substr($utility->getNomeCadastro($row->sal_prf_codigo, "profissionais"),0,32),0,1,'L');

			$this->SetXY(252, $ini);
			$this->Cell(35,5,Utility::formataData($row->sal_datasaida),0,1,'C');

			if ($_SESSION["rel_alm_codigo"] > 0) {
				$aux = " AND i.isa_alm_codigo = ".$_SESSION["rel_alm_codigo"];
			} else {
				$aux = "";
			}
			...
			$sql = "SELECT i.* FROM itenssaidasalmoxarifados i
					...
					$aux
					ORDER BY i.isa_codigo";

			$params = array();
			array_push($params, array('name'=>'CodigoPrefeitura','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
			array_push($params, array('name'=>'sal_codigo',      'value'=>$row->sal_codigo, 'type'=>PDO::PARAM_INT));
			...
			while ($rowitens = $objQryItens->fetch(PDO::FETCH_OBJ)) {
				...
				$this->SetXY(42, $ini);
				$this->Cell(134,5,substr($utility->getNomeCadastro($rowitens->isa_alm_codigo, "almoxarifados"),0,80),0,1,'L');

				$this->SetXY(178, $ini);
				$this->Cell(35,5,$rowitens->isa_qtd,0,1,'R');

				$this->SetXY(217, $ini);
				$this->Cell(33,5,$rowitens->isa_lote,0,1,'L');

				$this->SetXY(252, $ini);
				$this->Cell(35,5,Utility::formataData($utility->getValidadeAlmoxarifadoLote($row->sal_uso_codigo, $rowitens->isa_alm_codigo, $rowitens->isa_lote)),0,1,'L');
			}
		}
	}
