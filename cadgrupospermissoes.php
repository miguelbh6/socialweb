<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROGRUPOSPERMISSSOES)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Grupos de Permissões");
	...
	$campos[1][0] = "gpe_codigo";
	$campos[1][1] = "Código do Grupo";

	$cad = new MCLASSGrid();

	$args = "";

	$cad->ordemdefault  = "gpe_codigo";
	$cad->tordemdefault = "asc";
	$cad->arqlis        = "cadgrupospermissoes.php";
	$cad->arqedt        = "newgrupospermissoes.php";
	$cad->MAX           = 50;
	$cad->init();
	$cad->localizar();
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		$gpe_codigo = $_POST['aux_id'];
		if ($utility->verificaCodigoCadastroExiste($gpe_codigo, "grupospermissoes")) {
			$params = array();
			array_push($params, array('name'=>'gpe_pre_codigo','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
			array_push($params, array('name'=>'gpe_codigo',    'value'=>$gpe_codigo,      'type'=>PDO::PARAM_INT));
			$sql = Utility::geraSQLDELETE("grupospermissoes", $params);
			$utility->executeSQL($sql, $params, true, true, true);
			Utility::setMsgPopup("Grupo de Permissão Excluído com Sucesso!", "success");
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirgrupopermissao&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Grupos de Permissões</legend>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("gpe_codigo", "Código do Grupo de Permissão", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("gpe_nome", "Nome do Grupo de Permissão", $args); ?>
			</th>
			<th width="40px">
			 	Excluir
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar    = $cad->arqedt."?acao=editar&uuid=".$row->gpe_uuid."&id=".$row->gpe_codigo;
	   $lkpermissoes = "cadgrupospermissoespermissoes.php?uuid=".$row->gpe_uuid."&id=".$row->gpe_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center">
		   	<a href="<?php echo $lkpermissoes; ?>">
				<img src="imagens/checklist.png" border="0" alt="Visualizar Nota Fiscal" title="Visualizar Nota Fiscal">
			</a>
		  </td>
		  <td align="center">
		   <?php echo $row->gpe_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->gpe_nome; ?>
		  </td>
		  <td align="center" valign="botton">
			<button class="form_btn_excluir" name="btnexcluir" id="btnexcluir"
			codigo="<?php echo $row->gpe_codigo; ?>"
			nome="<?php echo $row->gpe_nome; ?>"
