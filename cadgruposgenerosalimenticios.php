<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROGRUPOSGENEROSALIMENTICIOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Grupos de Gêneros Alimentícios");
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROGRUPOSGENEROSALIMENTICIOSEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROGRUPOSGENEROSALIMENTICIOSEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Grupo de Gênero Alimentício!", "danger");
		} else {
			$gga_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($gga_codigo, "gruposgenerosalimenticios")) {
				$params = array();
				array_push($params, array('name'=>'gga_pre_codigo','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'gga_codigo',    'value'=>$gga_codigo,      'type'=>PDO::PARAM_INT));
				$sql = Utility::geraSQLDELETE("gruposgenerosalimenticios", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				Utility::setMsgPopup("Grupo de Gênero Alimentício Excluído com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirgrupogeneroalimenticio&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Grupos de Gêneros Alimentícios</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Grupos de Gêneros Alimentícios</legend>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("gga_codigo", "Código do Grupo", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("gga_nome", "Nome do Grupo", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->gga_uuid."&id=".$row->gga_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center">
		   <?php echo $row->gga_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->gga_nome; ?>
		  </td>

		  <td align="center" valign="botton">
			<button class="form_btn_excluir" name="btnexcluir" id="btnexcluir"
			codigo="<?php echo $row->gga_codigo; ?>"
			nome="<?php echo $row->gga_nome; ?>"
