<?php
	...
	global $PER_CADASTRONATUREZAS;
	if (!$utility->usuarioPermissao($PER_CADASTRONATUREZAS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Naturezas - 1");
	...
	$cad = new MCLASSGrid();
	$cad->arqlis = "cadnaturezas.php";
	$cad->arqedt = "newnaturezas.php";

	function getDados() {
		foreach($_POST as $key =>$val) {
			if (substr($key, 0, 4) == 'nat_') {
				global $$key;
	...
	function validaDados() {
		global $msg, $utility, $nat_nome, $nat_codigo;
		$msg = "";

		//Nome
		if (Utility::Vazio($nat_nome)) {
			$msg = "Nome da Natureza Inválido";
		}
		//if ((Utility::Vazio($msg)) && (strlen($nat_nome) < 10)) {
		//	$msg = "Nome da Natureza Inválido(Poucos caracteres)";
		//}
		if ((Utility::Vazio($msg)) && ($utility->verificaNomeCadastroExiste($nat_nome, $nat_codigo, "naturezas"))) {
			$msg = "Nome da Natureza Já Existe no Cadastro";
		}
		return;
	}
	...
	if ($_GET['acao'] == "inserir") {
		...
		global $PER_CADASTRONATUREZASINSERIR;
		if (!$utility->usuarioPermissao($PER_CADASTRONATUREZASINSERIR)) {
			Utility::setMsgPopup("Você não tem acesso a inserir Natureza!", "danger");
			Utility::redirect($cad->arqlis);
		}
		...
		//Inserir - Salvar
		if ((isset($_POST['nat_nome'])) && ($subacao == "salvar")) {
			getDados();
			validaDados();
			...
			if (Utility::Vazio($msg)) {
				...
				$UltimoCodigo  = $utility->getProximoCodigoTabela("naturezas");
				...
				$params = array();
				array_push($params, array('name'=>'nat_codigo',      'value'=>$id,              'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'nat_pre_codigo',  'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'nat_uuid',        'value'=>$uuid,            'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'nat_usu_cadastro','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'nat_datacadastro','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'nat_nome',        'value'=>$nat_nome,        'type'=>PDO::PARAM_STR));

				$sql = Utility::geraSQLINSERT("naturezas", $params);
	...
	if ($_GET['acao'] == "editar") {
		...
		if ($numrows != 1) {
			$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Naturezas - 2");
			Utility::redirect("acessonegado.php");
		}
		...
		//Alterar - Salvar
		if ((isset($_POST['nat_nome'])) && ($subacao == "salvar")) {
			global $PER_CADASTRONATUREZASALTERAR;
			if (!$utility->usuarioPermissao($PER_CADASTRONATUREZASALTERAR)) {
				Utility::setMsgPopup("Você não tem acesso a alterar Natureza!", "danger");
				Utility::redirect($cad->arqlis);
			}
			...
			if (Utility::Vazio($msg)) {
				...
				$params = array();
				array_push($params, array('name'=>'nat_nome',         'value'=>$nat_nome,        'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'nat_usu_alteracao','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT,'operador'=>'SET'));
				array_push($params, array('name'=>'nat_dataalteracao','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'nat_pre_codigo',   'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));
				array_push($params, array('name'=>'nat_uuid',         'value'=>$uuid,            'type'=>PDO::PARAM_STR,'operador'=>'WHERE'));
				array_push($params, array('name'=>'nat_codigo',       'value'=>$id,              'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));

				$sql = Utility::geraSQLUPDATE("naturezas", $params);
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Naturezas<?php echo $textoacao; ?></div>
		<br/>
...
<fieldset style="width:730px;" class="classfieldset1">
   <legend class="classlegend1">Dados da Natureza</legend>
...
<table border="0" width="100%" cellspacing="10" cellpadding="0" align="center">
 <tr>
  <td align="left">
    <label class="classlabel1">Código da Natureza:&nbsp;</label>
  	<input type="text" maxlength="100" name="nat_codigo" id="nat_codigo" disabled="disabled" class="classinput1" style="width:150px;text-align:right;background-color:#bcbcbc;" value="<?php echo $nat_codigo; ?>">
  </td>
 </tr>

 <tr>
  <td align="left">
    <label class="classlabel1">Nome da Natureza:&nbsp;<label class="labelasterisco">*&nbsp;</label></label>
  	<input type="text" maxlength="50" name="nat_nome" id="nat_nome" class="classinput1 inputobrigatorio" style="width:450px" value="<?php echo $nat_nome; ?>">
  </td>
 </tr>
