<?php
	...
	global $PER_CADASTRODESTINOS;
	if (!$utility->usuarioPermissao($PER_CADASTRODESTINOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Origem/Destino - 1");
	...
	$cad = new MCLASSGrid();
	$cad->arqlis = "caddestinos.php";
	$cad->arqedt = "newdestinos.php";

	function getDados() {
		foreach($_POST as $key =>$val) {
			if (substr($key, 0, 4) == 'des_') {
				global $$key;
	...
	function validaDados() {
		global $msg, $utility, $des_nome, $des_codigo;
		$msg = "";

		//Nome
		if (Utility::Vazio($des_nome)) {
			$msg = "Nome da Origem/Destino Inválido";
		}
		//if ((Utility::Vazio($msg)) && (strlen($des_nome) < 10)) {
		//	$msg = "Nome da Origem/Destino Inválido(Poucos caracteres)";
		//}
		if ((Utility::Vazio($msg)) && ($utility->verificaNomeCadastroExiste($des_nome, $des_codigo, "destinos"))) {
			$msg = "Nome da Origem/Destino Já Existe no Cadastro";
		}
		return;
	}
	...
	if ($_GET['acao'] == "inserir") {
		global $PER_CADASTRODESTINOSINSERIR;
		if (!$utility->usuarioPermissao($PER_CADASTRODESTINOSINSERIR)) {
			Utility::setMsgPopup("Você não tem acesso a inserir Origem/Destino!", "danger");
			Utility::redirect($cad->arqlis);
		}
		...
		//Inserir - Salvar
		if ((isset($_POST['des_nome'])) && ($subacao == "salvar")) {
			getDados();
			validaDados();
			...
			if (Utility::Vazio($msg)) {
				...
				$UltimoCodigo  = $utility->getProximoCodigoTabela("destinos");
				...
				$params = array();
				array_push($params, array('name'=>'des_codigo',      'value'=>$id,              'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'des_pre_codigo',  'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'des_uuid',        'value'=>$uuid,            'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'des_usu_cadastro','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'des_datacadastro','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'des_nome',        'value'=>$des_nome,        'type'=>PDO::PARAM_STR));

				$sql = Utility::geraSQLINSERT("destinos", $params);
	...
	if ($_GET['acao'] == "editar") {
		...
		//Carrega Dados
		$sql = "SELECT c.* FROM destinos c
				WHERE c.des_pre_codigo = :CodigoPrefeitura
				AND   c.des_codigo     = :id
				AND   c.des_uuid       = :uuid";
		...
		if ($numrows != 1) {
			$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Destino - 2");
			Utility::redirect("acessonegado.php");
		}
		...
		//Alterar - Salvar
		if ((isset($_POST['des_nome'])) && ($subacao == "salvar")) {
			global $PER_CADASTRODESTINOSALTERAR;
			if (!$utility->usuarioPermissao($PER_CADASTRODESTINOSALTERAR)) {
				Utility::setMsgPopup("Você não tem acesso a alterar Origem/Destino!", "danger");
				Utility::redirect($cad->arqlis);
			}
			...
			if (Utility::Vazio($msg)) {
				...
				$params = array();
				array_push($params, array('name'=>'des_nome',         'value'=>$des_nome,        'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'des_usu_alteracao','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT,'operador'=>'SET'));
				array_push($params, array('name'=>'des_dataalteracao','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'des_pre_codigo',   'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));
				array_push($params, array('name'=>'des_uuid',         'value'=>$uuid,            'type'=>PDO::PARAM_STR,'operador'=>'WHERE'));
				array_push($params, array('name'=>'des_codigo',       'value'=>$id,              'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));

				$sql = Utility::geraSQLUPDATE("destinos", $params);
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Origens/Destinos<?php echo $textoacao; ?></div>
		<br/>
...
<fieldset style="width:730px;" class="classfieldset1">
   <legend class="classlegend1">Dados da Origem/Destino</legend>
...
<table border="0" width="100%" cellspacing="10" cellpadding="0" align="center">
 <tr>
  <td align="left">
    <label class="classlabel1">Código da Origem/Destino:&nbsp;</label>
  	<input type="text" maxlength="100" name="des_codigo" id="des_codigo" disabled="disabled" class="classinput1" style="width:150px;text-align:right;background-color:#bcbcbc;" value="<?php echo $des_codigo; ?>">
  </td>
 </tr>

 <tr>
  <td align="left">
    <label class="classlabel1">Nome da Origem/Destino:&nbsp;<label class="labelasterisco">*&nbsp;</label></label>
  	<input type="text" maxlength="50" name="des_nome" id="des_nome" class="classinput1 inputobrigatorio" style="width:450px" value="<?php echo $des_nome; ?>">
  </td>
 </tr>
