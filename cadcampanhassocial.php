<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROCAMPANHASSOCIAL)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Campanhas Social");
	...
	$campos[0][0] = "cso_nome";
	$campos[0][1] = "Nome da Campanha";

	$cad = new MCLASSGrid();

	$args = "";

	$cad->ordemdefault  = "cso_nome";
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROCAMPANHASSOCIALEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROCAMPANHASSOCIALEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Campanhas Social!", "danger");
		} else {
			$cso_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($cso_codigo, "campanhassocial")) {
				$params = array();
				array_push($params, array('name'=>'cso_pre_codigo','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'cso_codigo',    'value'=>$cso_codigo,      'type'=>PDO::PARAM_INT));
				$sql = Utility::geraSQLDELETE("campanhassocial", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				Utility::setMsgPopup("Campanha Social Excluída com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluircampanhasocial&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Campanhas Social</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Campanhas Social</legend>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("cso_codigo", "Código da Campanha", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("cso_nome", "Nome da Campanha", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->cso_uuid."&id=".$row->cso_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center">
		   <?php echo $row->cso_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->cso_nome; ?>
		  </td>

		  <td align="center" valign="botton">
			<button class="form_btn_excluir" name="btnexcluir" id="btnexcluir"
			codigo="<?php echo $row->cso_codigo; ?>"
			nome="<?php echo $row->cso_nome; ?>"
