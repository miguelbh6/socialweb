...
class Rel1SaidaMateriaisDidaticosPDF extends tFPDF {
	...
	private function geraCabecalho() {
		...
		if (!isset($_SESSION["rel_smd_dataini"])) {
			return;
		}
		...
		$this->Cell(278,5,'RELATÓRIO DE SAÍDAS DE MATERIAIS DIDÁTICOS',0,1,'C');
	...
	function geraRelatorio() {
		...
		if (!isset($_SESSION["rel_smd_sql"])) {
			return;
		}

		$sql = $_SESSION["rel_smd_sql"];
		...
		while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
			...
			$this->SetXY(10, $ini);
			$this->Cell(20,5,$row->smd_codigo,0,1,'R');

			$this->SetXY(32, $ini);
			$this->Cell(90,5,substr($utility->getNomeCadastro($row->smd_prf_codigo, "profissionais"),0,50),0,1,'L');

			//$this->SetXY(124, $ini);
			//$this->Cell(64,5,substr($utility->getValorCadastroCampo($row->smd_prf_codigo, "profissionais", "prf_endereco"),0,40),0,1,'L');

			//$this->SetXY(190, $ini);
			//$this->Cell(60,5,substr($utility->getNomeCadastro($row->smd_prf_codigo, "profissionais"),0,32),0,1,'L');

			$this->SetXY(252, $ini);
			$this->Cell(35,5,Utility::formataData($row->smd_datasaida),0,1,'C');

			if ($_SESSION["rel_mdi_codigo"] > 0) {
				$aux = " AND i.ism_mdi_codigo = ".$_SESSION["rel_mdi_codigo"];
			} else {
				$aux = "";
			}

			$sql = "SELECT i.* FROM itenssaidasmateriaisdidaticos i
					WHERE i.ism_pre_codigo = :CodigoPrefeitura
					AND   i.ism_smd_codigo = :smd_codigo
					$aux
					ORDER BY i.ism_codigo";

			$params = array();
			array_push($params, array('name'=>'CodigoPrefeitura','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
			array_push($params, array('name'=>'smd_codigo',      'value'=>$row->smd_codigo, 'type'=>PDO::PARAM_INT));
			...
			while ($rowitens = $objQryItens->fetch(PDO::FETCH_OBJ)) {
				...
				$this->SetXY(42, $ini);
				$this->Cell(134,5,substr($utility->getNomeCadastro($rowitens->ism_mdi_codigo, "materiaisdidaticos"),0,80),0,1,'L');

				$this->SetXY(178, $ini);
				$this->Cell(35,5,$rowitens->ism_qtd,0,1,'R');

				$this->SetXY(217, $ini);
				$this->Cell(33,5,$rowitens->ism_lote,0,1,'L');

				$this->SetXY(252, $ini);
				$this->Cell(35,5,Utility::formataData($utility->getValidadeMaterialDidaticoLote($row->smd_uso_codigo, $rowitens->ism_mdi_codigo, $rowitens->ism_lote)),0,1,'L');
			}
		}
	}
