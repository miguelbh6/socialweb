<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROSAIDASGENEROSALIMENTICIOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Saídas de Gêneros Alimentícios");
		Utility::redirect("acessonegado.php");
	}

	$utility->SalvaCadastroAcessado(basename($_SERVER['SCRIPT_FILENAME']), "Cadastro de Saídas de Gêneros Alimentícios");
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROSAIDASGENEROSALIMENTICIOSEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROSAIDASGENEROSALIMENTICIOSEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Saídas de Gêneros Alimentícios!", "danger");
		} else {
			$sga_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($sga_codigo, "saidasgenerosalimenticios")) {
				//$listagenerosalimenticios = $utility->getListaSaidaGenerosAlimenticiosAtualizacaoEstoque($sga_codigo);
				...
				$sql = Utility::geraSQLDELETE("saidasgenerosalimenticios", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				...
				Utility::setMsgPopup("Saída de Gêneros Alimentícios Excluída com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirsaidagenerosalimenticios&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
$(".form_btn_itens").on("click", function(e) {
	...
	$("#aux_sga_codigo").val(codigo);
	$("#aux_sga_uuid").val(uuid);

	var url = 'ajax_itenssaidasgenerosalimenticios.php?id=' + codigo + '&uuid=' + uuid + '&time=' + $.now();
...
$("#linkrel1").on("click", function(e) {
	...
	$.ajax({
	url: "processajax.php?acao=gerarrel1saidagenerosalimenticios&arq=" + arq1 + "&time=" + $.now(),
	type: "get",
	dataType: "json",
	success: function(response) {
		sleep(100);
...
$("#linkrel2").on("click", function(e) {
	...
	$.ajax({
	url: "processajax.php?acao=gerarrelsintetico1saidagenerosalimenticios&arq=" + arq1 + "&time=" + $.now(),
	type: "get",
	dataType: "json",
	success: function(response) {
		sleep(100);
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Saídas de Gêneros Alimentícios</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Saídas de Gêneros Alimentícios</legend>
...
<?php
	...
	if ($unidadesocial > 0)
		$unidade = "AND s.sga_uso_codigo = ".$unidadesocial;
	else
		$unidade = "";

	if ($gal_codigo > 0)
		$strgal_codigo = "AND s.sga_codigo IN (SELECT i.isg_sga_codigo FROM itenssaidasgenerosalimenticios i WHERE i.isg_pre_codigo = $CodigoPrefeitura AND i.isg_gal_codigo = $gal_codigo)";
	else
		$strgal_codigo = "";
	...
	$_SESSION["rel_sga_sql"]        = $sql;
	$_SESSION["rel_sga_dataini"]    = $dataini;
	$_SESSION["rel_sga_datafim"]    = $datafim;
	$_SESSION["rel_sga_uso_codigo"] = $unidadesocial;
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px" nowrap>
			 	<?php echo $cad->geraURLTitulo("sga_codigo", "Código da Saída", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("sga_prf_codigo", "Profissional da Saída", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("sga_datasaida", "Data da Saída", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("sga_uso_codigo", "Unidade Social", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->sga_uuid."&id=".$row->sga_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center" valign="botton">
			...
			codigo="<?php echo $row->sga_codigo; ?>"
			uuid="<?php echo $row->sga_uuid; ?>"
			type="button" style="border: 0; background: transparent"><img src="imagens/view1.png"/>
			</button>
	     </td>

		  <td align="center">
		   <?php echo $row->sga_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $utility->getNomeCadastro($row->sga_prf_codigo, "profissionais"); ?>
		  </td>
		  </td>
		  <td align="center">
		   &nbsp;<?php echo Utility::formataData($row->sga_datasaida); ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $utility->getNomeCadastro($row->sga_uso_codigo, "unidadessocial"); ?>
		  </td>

		  <td align="center" valign="botton">
			<button class="form_btn_excluir" name="btnexcluir" id="btnexcluir"
			codigo="<?php echo $row->sga_codigo; ?>"
			nome="<?php echo $utility->getNomeCadastro($row->sga_prf_codigo, "profissionais"); ?>"
...
<!-- Tela de Itens -->
<div id="telaitens_tela" title="Itens da Saída de Gêneros Alimentícios">
<input type="hidden" name="aux_sga_codigo" id="aux_sga_codigo"/>
<input type="hidden" name="aux_sga_uuid"   id="aux_sga_uuid"/>
<div id="itens_tela"></div>
</div>
<!-- Tela de Itens -->

<!-- Tela de Relatórios -->
<div id="telarelatorios_tela" title="Relatórios - Saídas de Gêneros Alimentícios">
<table width="80%" id="customers" border="1" align="center" style="background-color:#ebf5fe;">
	<tr>
				...
				<td style="border:0px;" align="left">
					<a href="#" class="button_rel1 button_rel1-blue">
						...
						<span style="width:350px" id="linkrel1"
							arq1="<?php echo "rel1saidagenerosalimenticios_".$rel_uuid.".pdf"; ?>"
							arq2="<?php echo Utility::getPathDownPDF()."rel1saidagenerosalimenticios_".$rel_uuid.".pdf"; ?>">
						Relatório de Saídas de Gêneros Alimentícios
						</span>
					</a>
				</td>
			</tr>
				...
				<td style="border:0px;" align="left">
					<a href="#" class="button_rel1 button_rel1-blue">
						...
						<span style="width:350px" id="linkrel2"
							arq1="<?php echo "relsintetico1saidagenerosalimenticios_".$rel_uuid.".pdf"; ?>"
							arq2="<?php echo Utility::getPathDownPDF()."relsintetico1saidagenerosalimenticios_".$rel_uuid.".pdf"; ?>">Relatório Sintético de Saídas de Gêneros Alimentícios</span>
					</a>
				</td>
			</tr>
			</table>
