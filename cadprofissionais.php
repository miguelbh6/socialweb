<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROPROFISSIONALSOCIAL)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Profissional Social");
		Utility::redirect("acessonegado.php");
	}

	$utility->SalvaCadastroAcessado(basename($_SERVER['SCRIPT_FILENAME']), "Cadastro de Profissional Social");
	...
	$campos[0][0] = "prf_nome";
	$campos[0][1] = "Nome do Profissional";
	$campos[1][0] = "prf_rg";
	$campos[1][1] = "RG do Profissional";
	$campos[2][0] = "prf_cpf";
	$campos[2][1] = "CPF do Profissional";
	$campos[3][0] = "prf_celular";
	$campos[3][1] = "CPF do Profissional";
	$campos[4][0] = "cpr_nome";
	$campos[4][1] = "Cargo/Função";

	$cad = new MCLASSGrid();

	$args = "";

	$cad->ordemdefault  = "prf_nome";
	...
	//Remove Máscara
	if ($campo == 'prf_cpf') {
		$strconsulta = Utility::somenteNumeros($strconsulta);
	}
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROPROFISSIONALSOCIALEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROPROFISSIONALSOCIALEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Profissional Social!", "danger");
		} else {
			$prf_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($prf_codigo, "profissionais")) {
				$params = array();
				...
				$sql = Utility::geraSQLDELETE("profissionais", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				Utility::setMsgPopup("Profissional Social Excluído com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirprofissional&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Profissional Social</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Profissional Social</legend>
...
<?php
	...
	$sql = "SELECT p.*, c.cpr_nome FROM profissionais p LEFT JOIN cargosprofissionais c
			ON p.prf_cpr_codigo = c.cpr_codigo AND p.prf_pre_codigo = c.cpr_pre_codigo
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("prf_codigo", "Código do Profissional", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("prf_nome", "Nome do Profissional", $args); ?>
			</th>
			<th width="80px">
			 	<?php echo $cad->geraURLTitulo("cpr_nome", "Cargo/Função", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("prf_endereco", "Endereço do Profissional", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("prf_celular", "Celular", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->prf_uuid."&id=".$row->prf_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center">
		   <?php echo $row->prf_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->prf_nome; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->cpr_nome; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->prf_endereco; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->prf_celular; ?>
		  </td>

		  <td align="center" valign="botton">
			<button class="form_btn_excluir" name="btnexcluir" id="btnexcluir"
			codigo="<?php echo $row->prf_codigo; ?>"
			nome="<?php echo $row->prf_nome; ?>"
