<?php
	...
	global $PER_CADASTROSAIDASGENEROSALIMENTICIOS;
	if (!$utility->usuarioPermissao($PER_CADASTROSAIDASGENEROSALIMENTICIOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Saída de Gêneros Alimentícios - 1");
	...
	$cad = new MCLASSGrid();
	$cad->arqlis = "cadsaidasgenerosalimenticios.php";
	$cad->arqedt = "newsaidasgenerosalimenticios.php";

	function getDados() {
		foreach($_POST as $key =>$val) {
			if (substr($key, 0, 4) == 'sga_') {
				global $$key;
	...
	function formataDados() {
		global $sga_datasaida, $sga_prf_codigo;

		if ((Utility::Vazio($sga_prf_codigo)) || ($sga_prf_codigo == "0")) {
			$sga_prf_codigo = 'NULL';
		}

		if (!Utility::Vazio($sga_datasaida))
			$sga_datasaida = Utility::formataDataMysql($sga_datasaida);
		else
			$sga_datasaida = 'NULL';
	...
	function validaDados() {
		global $msg, $utility, $sga_codigo, $sga_prf_codigo, $sga_uso_codigo, $sga_datasaida;
		$msg = "";
		...
		//Unidade Social
		if ((Utility::Vazio($msg)) && (Utility::Vazio($sga_uso_codigo)) || ($sga_uso_codigo == "0")) {
			$msg = "Favor Selecionar a Unidade Social";
		}

		//Data da Saida
		if ((Utility::Vazio($msg)) && (Utility::Vazio($sga_datasaida))) {
			$msg = "Favor Informar a Data da Saída";
		}
		if ((Utility::Vazio($msg)) && (!Utility::validaData($sga_datasaida))) {
			$msg = "Data da Saída Inválida";
		}
		$ano = substr($sga_datasaida, 6, 4);
	...
	if ($_GET['acao'] == "inserir") {
		global $PER_CADASTROSAIDASGENEROSALIMENTICIOSINSERIR;
		if (!$utility->usuarioPermissao($PER_CADASTROSAIDASGENEROSALIMENTICIOSINSERIR)) {
			Utility::setMsgPopup("Você não tem acesso a inserir Saída de Gêneros Alimentícios!", "danger");
			Utility::redirect($cad->arqlis);
		}
		...
		//Campos
		$sga_codigo        = "";
		$sga_uso_codigo    = $_SESSION['fuso_codigo'];
		$sga_prf_codigo    = "";
		$sga_datasaida     = Utility::formataData($utility->getData());
		$sga_obs           = "";
		$sga_datacadastro  = "";
        $sga_usu_cadastro  = "";
        $sga_dataalteracao = "";
		...
		//Inserir - Salvar
		if ((isset($_POST['sga_prf_codigo'])) && ($subacao == "salvar")) {
			getDados();
			validaDados();
			...
			if (Utility::Vazio($msg)) {
				...
				$UltimoCodigo  = $utility->getProximoCodigoTabela("saidasgenerosalimenticios");
				...
				$params = array();
				array_push($params, array('name'=>'sga_codigo',      'value'=>$id,              'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'sga_pre_codigo',  'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'sga_uuid',        'value'=>$uuid,            'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'sga_usu_cadastro','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'sga_datacadastro','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'sga_prf_codigo',  'value'=>$sga_prf_codigo,  'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'sga_uso_codigo',  'value'=>$sga_uso_codigo,  'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'sga_datasaida',   'value'=>$sga_datasaida,   'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'sga_obs',         'value'=>$sga_obs,         'type'=>PDO::PARAM_STR));

				$sql = Utility::geraSQLINSERT("saidasgenerosalimenticios", $params);
	...
	if ($_GET['acao'] == "editar") {
		...
		//Carrega Dados
		$sql = "SELECT s.* FROM saidasgenerosalimenticios s
		...
		if ($numrows != 1) {
			$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Saída de Gêneros Alimentícios - 2");
			Utility::redirect("acessonegado.php");
		}
		...
		$sga_datasaida = Utility::formataData($sga_datasaida);

		//Alterar - Salvar
		if ((isset($_POST['sga_prf_codigo'])) && ($subacao == "salvar")) {
			global $PER_CADASTROSAIDASGENEROSALIMENTICIOSALTERAR;
			if (!$utility->usuarioPermissao($PER_CADASTROSAIDASGENEROSALIMENTICIOSALTERAR)) {
				Utility::setMsgPopup("Você não tem acesso a alterar Saída de Gêneros Alimentícios!", "danger");
				Utility::redirect($cad->arqlis);
			}
			...
			if (Utility::Vazio($msg)) {
				...
				$params = array();
				array_push($params, array('name'=>'sga_uso_codigo',   'value'=>$sga_uso_codigo,  'type'=>PDO::PARAM_INT,'operador'=>'SET'));
				array_push($params, array('name'=>'sga_prf_codigo',   'value'=>$sga_prf_codigo,  'type'=>PDO::PARAM_INT,'operador'=>'SET'));
				array_push($params, array('name'=>'sga_datasaida',    'value'=>$sga_datasaida,   'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'sga_obs',          'value'=>$sga_obs,         'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'sga_usu_alteracao','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT,'operador'=>'SET'));
				array_push($params, array('name'=>'sga_dataalteracao','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'sga_pre_codigo',   'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));
				array_push($params, array('name'=>'sga_uuid',         'value'=>$uuid,            'type'=>PDO::PARAM_STR,'operador'=>'WHERE'));
				array_push($params, array('name'=>'sga_codigo',       'value'=>$id,              'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));

				$sql = Utility::geraSQLUPDATE("saidasgenerosalimenticios", $params);
...
var url = 'ajax_itenssaidasgenerosalimenticios.php?id=<?php echo $id; ?>&uuid=<?php echo $uuid; ?>&time=' + $.now();
$.get(url, function(dataReturn) {
	$('#itens_tela').html(dataReturn);
});

$("#listatabs").tabs();
$("#sga_prf_codigo").focus();
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Saídas de Gêneros Alimentícios<?php echo $textoacao; ?></div>
		<br/>
...
<div id="listatabs">
		    <ul style="background:#ffffff">
                <li class="wizardulli" style="width:35%;"><a href="#pag1" class="wizarda"><span class="wizardnumber">1.&nbsp;</span>Dados da Saída de Gêneros Alimentícios</a></li>
...
<fieldset style="width:700px;" class="classfieldset1">
   <legend class="classlegend1">Dados da Saída de Gêneros Alimentícios</legend>
...
<table border="0" width="100%" cellspacing="10" cellpadding="0" align="center">
 <tr>
  <td align="left">
    ...
    <tr>
		<td>
			<label class="classlabel1">Código da Saída de Gêneros Alimentícios:&nbsp;</label>
  			<input type="text" maxlength="100" name="sga_codigo" id="sga_codigo" disabled="disabled" class="classinput1" style="width:150px;text-align:right;background-color:#bcbcbc;" value="<?php echo $sga_codigo; ?>">
		</td>
    </tr>
    </table>
  </td>
...
?>
 ...
 <td align="left">
    <label class="classlabel1">Profissional da Saída:&nbsp;<label class="labelasterisco">*&nbsp;</label></label>
  	<select name="sga_prf_codigo" id="sga_prf_codigo" style="width:563px" class="selectform inputobrigatorio">
		...
		<?php
			...
			while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
				if ($sga_prf_codigo == $row->prf_codigo)
 ...
 <td align="left">
    <label class="classlabel1">Unidade Social:&nbsp;<label class="labelasterisco">*&nbsp;</label></label>
  	<select name="sga_uso_codigo" id="sga_uso_codigo" style="width:563px" class="selectform inputobrigatorio">
		<option value="0" selected="selected"></option>
		<?php
			$CodigoPrefeitura = Utility::getCodigoPrefeitura();
			...
			$objQry = $utility->querySQL($sql, $params);
			while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
				if ($sga_uso_codigo == $row->uso_codigo)
					$aux = "selected='selected'";
			    else
					$aux = "";
 ...
 <tr>
  <td align="left">
    <label class="classlabel1">Data da Saída:&nbsp;<label class="labelasterisco">*&nbsp;</label></label>
  	<input type="text" maxlength="15" name="sga_datasaida" id="sga_datasaida" class="classinput1 datemask inputobrigatorio" style="width:250px" value="<?php echo $sga_datasaida; ?>">
  </td>
 </tr>
...
<textarea name="sga_obs" id="sga_obs" class="classinput1" rows="3" style="width:550px;font-size:12px;">
<?php echo $sga_obs; ?>
</textarea>
  </td>
 </tr>
