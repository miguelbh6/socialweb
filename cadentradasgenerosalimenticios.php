<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROENTRADASGENEROSALIMENTICIOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Entradas de Gêneros Alimentícios");
		Utility::redirect("acessonegado.php");
	}

	$utility->SalvaCadastroAcessado(basename($_SERVER['SCRIPT_FILENAME']), "Cadastro de Entradas de Gêneros Alimentícios");
	...
	$campos[0][0] = "for_nome";
	$campos[0][1] = "Nome do Fornecedor";
	$campos[1][0] = "ega_codigo";
	$campos[1][1] = "Código da Entrada";
	$campos[2][0] = "ega_numeronota";
	$campos[2][1] = "Número da Nota Fiscal";
	$campos[3][0] = "ega_serienota";
	$campos[3][1] = "Série da Nota";

	$cad = new MCLASSGrid();

	$args = "";

	$cad->ordemdefault  = "ega_codigo";
	...
	$gal_nome = (isset($_POST["gal_nome"]))? $_POST["gal_nome"] : "";

	if (Utility::Vazio($gal_nome)) {
		$gal_codigo = 0;
	}

	$gal_nome = $utility->getNomeCadastro($gal_codigo, "generosalimenticios");
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROENTRADASGENEROSALIMENTICIOSEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROENTRADASGENEROSALIMENTICIOSEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Entradas de Gêneros Alimentícios!", "danger");
		} else {
			$ega_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($ega_codigo, "entradasgenerosalimenticios")) {
				//$listagenerosalimenticios = $utility->getListaEntradaGenerosAlimenticiosAtualizacaoEstoque($ega_codigo);
				...
				$sql = Utility::geraSQLDELETE("entradasgenerosalimenticios", $params);
				$utility->executeSQL($sql, $params, true, true, true);

				//foreach ($listagenerosalimenticios as &$gal_codigo) {
				//	$utility->atualizaEstoqueGeneroAlimenticio($gal_codigo);
				//}

				Utility::setMsgPopup("Entrada de Gêneros Alimentícios Excluída com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirentradagenerosalimenticios&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
$(".form_btn_itens").on("click", function(e) {
	...
	$("#aux_ega_codigo").val(codigo);
	$("#aux_ega_uuid").val(uuid);

	var url = 'ajax_itensentradasgenerosalimenticios.php?id=' + codigo + '&uuid=' + uuid + '&time=' + $.now();
...
$("#gal_nome").autocomplete({
	source: 'processajax.php?acao=getlistagenerosalimenticios&uso_codigo=0&time=' + $.now(),
	minLength: 5,
	selectFirst: true,
	open: function() {
		$('#ajaxBusy').hide();
...
$("#linkrel1").on("click", function(e) {
	...
	$.ajax({
	url: "processajax.php?acao=gerarrel1entradageneroalimenticio&arq=" + arq1 + "&time=" + $.now(),
	type: "get",
	dataType: "json",
	success: function(response) {
		sleep(100);
...
$("#linkrel2").on("click", function(e) {
	...
	$.ajax({
	url: "processajax.php?acao=gerarrelsintetico1entradageneroalimenticio&arq=" + arq1 + "&time=" + $.now(),
	type: "get",
	dataType: "json",
	success: function(response) {
		sleep(100);
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Entradas de Gêneros Alimentícios</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Entradas de Gêneros Alimentícios</legend>
...
<?php
	...
	if ($unidadesocial > 0)
		$unidade = "AND e.ega_uso_codigo = ".$unidadesocial;
	else
		$unidade = "";

	if ($gal_codigo > 0)
		$strgal_codigo = "AND e.ega_codigo IN (SELECT i.ieg_ega_codigo FROM itensentradasgenerosalimenticios i WHERE i.ieg_pre_codigo = $CodigoPrefeitura AND i.ieg_gal_codigo = $gal_codigo)";
	else
		$strgal_codigo = "";

	$sql = "SELECT e.*, f.for_nome FROM entradasgenerosalimenticios e LEFT JOIN fornecedores f
	...
	$_SESSION["rel_ega_sql"]        = $sql;
	$_SESSION["rel_ega_dataini"]    = $dataini;
	$_SESSION["rel_ega_datafim"]    = $datafim;
	$_SESSION["rel_ega_uso_codigo"] = $unidadesocial;
	$_SESSION["rel_gal_codigo"]     = $gal_codigo;
...
<tr>
	...
	<td align="center">
		...
		<table id="customers2" border="0" align="left" cellspacing="0" cellpadding="0">
		<tr>
		...
		<tr height="10px">
			<td colspan="4">
				<label class="classlabel1">Nome do Gênero Alimentício:&nbsp;</label>
				<input type="hidden" name="gal_codigo" id="gal_codigo" value="<?php echo $gal_codigo; ?>"/>
				<input type="text" maxlength="100" name="gal_nome" id="gal_nome" class="classinput1" style="width:450px" value="<?php echo $gal_nome; ?>">
				<span style="font-size:10px;">Informe o nome do gênero alimentício com no mínimo de 5 caracteres.</span>
			</td>
		</tr>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("ega_codigo", "Código da Entrada", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("for_nome", "Nome do Fornecedor", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("ega_dataentrada", "Data da Entrada", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("ega_uso_codigo", "Unidade Social", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("ega_dataemissao", "Data da Emissão", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("ega_numeronota", "Número da Nota Fiscal", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("ega_serienota", "Série da Nota Fiscal", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->ega_uuid."&id=".$row->ega_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center" valign="botton">
			...
			codigo="<?php echo $row->ega_codigo; ?>"
			uuid="<?php echo $row->ega_uuid; ?>"
			type="button" style="border: 0; background: transparent"><img src="imagens/view1.png"/>
			</button>
	      </td>

		  <td align="center">
		   <?php echo $row->ega_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->for_nome; ?>
		  </td>
		  </td>
		  <td align="center">
		   &nbsp;<?php echo Utility::formataData($row->ega_dataentrada); ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $utility->getNomeCadastro($row->ega_uso_codigo, "unidadessocial"); ?>
		  </td>
		  <td align="center">
		   &nbsp;<?php echo Utility::formataData($row->ega_dataemissao); ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->ega_numeronota; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->ega_serienota; ?>
		  </td>

		  <td align="center" valign="botton">
			<button class="form_btn_excluir" name="btnexcluir" id="btnexcluir"
			codigo="<?php echo $row->ega_codigo; ?>"
			nome="<?php echo $row->for_nome; ?>"
...
<!-- Tela de Itens -->
<div id="telaitens_tela" title="Itens da Entrada de Gêneros Alimentícios">
<input type="hidden" name="aux_ega_codigo" id="aux_ega_codigo"/>
<input type="hidden" name="aux_ega_uuid"   id="aux_ega_uuid"/>
<div id="itens_tela"></div>
</div>
<!-- Tela de Itens -->

<!-- Tela de Relatórios -->
<div id="telarelatorios_tela" title="Relatórios - Entradas de Gêneros Alimentícios">
<table width="80%" id="customers" border="1" align="center" style="background-color:#ebf5fe;">
	<tr>
				...
				<td style="border:0px;" align="left">
					<a href="#" class="button_rel1 button_rel1-blue">
						...
						<span style="width:350px" id="linkrel1"
							arq1="<?php echo "rel1entradageneroalimenticio_".$rel_uuid.".pdf"; ?>"
							arq2="<?php echo Utility::getPathDownPDF()."rel1entradageneroalimenticio_".$rel_uuid.".pdf"; ?>">
						Relatório de Entradas de Gêneros Alimentícios
						</span>
					</a>
				</td>
			</tr>
				...
				<td style="border:0px;" align="left">
					<a href="#" class="button_rel1 button_rel1-blue">
						...
						<span style="width:350px" id="linkrel2"
							arq1="<?php echo "relsintetico1entradageneroalimenticio__".$rel_uuid.".pdf"; ?>"
							arq2="<?php echo Utility::getPathDownPDF()."relsintetico1entradageneroalimenticio__".$rel_uuid.".pdf"; ?>">Relatório Sintético de Entradas de Gêneros Alimentícios</span>
					</a>
				</td>
			</tr>
			</table>
