...
class RelSintetico1SaidaGenerosAlimenticiosPDF extends tFPDF {
	...
	private function geraCabecalho() {
		...
		$this->Cell(278,5,'RELATÓRIO SINTÉTICO DE SAÍDAS DE GÊNEROS ALIMENTÍCIOS',0,1,'C');
	...
	function geraRelatorio() {
		...
		if ($_SESSION["rel_gal_codigo"] > 0) {
			$aux = " AND a.gal_codigo = ".$_SESSION["rel_gal_codigo"];
		} else {
			$aux = "";
		}

		$sql = "SELECT a.gal_codigo, a.gal_nome FROM generosalimenticios a WHERE a.gal_pre_codigo = $CodigoPrefeitura $aux ORDER BY a.gal_nome";
		...
		while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
			$saidas = $utility->getTotalSaidasGenerosAlimenticiosDataUnidade($row->gal_codigo, $_SESSION["rel_sga_dataini"], $_SESSION["rel_sga_datafim"], $_SESSION["rel_sga_uso_codigo"]);
			if ($saidas > 0) {
				$ini = $this->GetY() + 1.3;
				$posY = $this->GetY();
				$i++;
