<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROUNIDADESMATERIAISDIDATICOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Unidades de Materiais Didáticos");
	...
	$campos[0][0] = "umd_nome";
	$campos[0][1] = "Nome da Unidade";

	$cad = new MCLASSGrid();

	$args = "";

	$cad->ordemdefault  = "umd_nome";
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROUNIDADESMATERIAISDIDATICOSEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROUNIDADESMATERIAISDIDATICOSEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Unidade de Material Didático!", "danger");
		} else {
			$umd_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($umd_codigo, "unidadesmateriaisdidaticos")) {
				$params = array();
				array_push($params, array('name'=>'umd_pre_codigo','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'umd_codigo',    'value'=>$umd_codigo,      'type'=>PDO::PARAM_INT));
				$sql = Utility::geraSQLDELETE("unidadesmateriaisdidaticos", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				Utility::setMsgPopup("Unidade de Material Didático Excluída com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirunidadematerialdidatico&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Unidades de Materiais Didáticos</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Unidades de Materiais Didáticos</legend>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("umd_codigo", "Código da Unidade", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("umd_nome", "Nome da Unidade", $args); ?>
			</th>
			<th width="40px">
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->umd_uuid."&id=".$row->umd_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center">
		   <?php echo $row->umd_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->umd_nome; ?>
		  </td>
		  <td align="left">
		  ...
		  <td align="center" valign="botton">
			...
			codigo="<?php echo $row->umd_codigo; ?>"
			nome="<?php echo $row->umd_nome; ?>"
