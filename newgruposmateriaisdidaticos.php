<?php
	...
	global $PER_CADASTROGRUPOSMATERIAISDIDATICOS;
	if (!$utility->usuarioPermissao($PER_CADASTROGRUPOSMATERIAISDIDATICOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Grupos de Materiais Didáticos - 1");
	...
	$cad = new MCLASSGrid();
	$cad->arqlis = "cadgruposmateriaisdidaticos.php";
	$cad->arqedt = "newgruposmateriaisdidaticos.php";

	function getDados() {
		foreach($_POST as $key =>$val) {
			if (substr($key, 0, 4) == 'gmd_') {
				global $$key;
	...
	function validaDados() {
		global $msg, $utility, $gmd_nome, $gmd_codigo;
		$msg = "";

		//Nome
		if (Utility::Vazio($gmd_nome)) {
			$msg = "Nome do Grupo de Material Didático Inválido";
		}
		//if ((Utility::Vazio($msg)) && (strlen($gmd_nome) < 10)) {
		//	$msg = "Nome do Grupo de Material Didático Inválido(Poucos caracteres)";
		//}
		if ((Utility::Vazio($msg)) && ($utility->verificaNomeCadastroExiste($gmd_nome, $gmd_codigo, "gruposmateriaisdidaticos"))) {
			$msg = "Nome do Grupo de Material Didático Já Existe no Cadastro";
		}
		return;
	}
	...
	if ($_GET['acao'] == "inserir") {
		global $PER_CADASTROGRUPOSMATERIAISDIDATICOSINSERIR;
		if (!$utility->usuarioPermissao($PER_CADASTROGRUPOSMATERIAISDIDATICOSINSERIR)) {
			Utility::setMsgPopup("Você não tem acesso a inserir Grupo de Material Didático!", "danger");
			Utility::redirect($cad->arqlis);
		}
		...
		//Inserir - Salvar
		if ((isset($_POST['gmd_nome'])) && ($subacao == "salvar")) {
			getDados();
			validaDados();
			...
			if (Utility::Vazio($msg)) {
				...
				$UltimoCodigo  = $utility->getProximoCodigoTabela("gruposmateriaisdidaticos");
				...
				$params = array();
				array_push($params, array('name'=>'gmd_codigo',      'value'=>$id,              'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'gmd_pre_codigo',  'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'gmd_uuid',        'value'=>$uuid,            'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'gmd_usu_cadastro','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'gmd_datacadastro','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'gmd_nome',        'value'=>$gmd_nome,        'type'=>PDO::PARAM_STR));

				$sql = Utility::geraSQLINSERT("gruposmateriaisdidaticos", $params);
	...
	if ($_GET['acao'] == "editar") {
		...
		//Carrega Dados
		$sql = "SELECT c.* FROM gruposmateriaisdidaticos c
				WHERE c.gmd_pre_codigo = :CodigoPrefeitura
				AND   c.gmd_codigo     = :id
				AND   c.gmd_uuid       = :uuid";
		...
		if ($numrows != 1) {
			$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Grupos de Materiais Didáticos - 2");
			Utility::redirect("acessonegado.php");
		}
		...
		//Alterar - Salvar
		if ((isset($_POST['gmd_nome'])) && ($subacao == "salvar")) {
			global $PER_CADASTROGRUPOSMATERIAISDIDATICOSALTERAR;
			if (!$utility->usuarioPermissao($PER_CADASTROGRUPOSMATERIAISDIDATICOSALTERAR)) {
				Utility::setMsgPopup("Você não tem acesso a alterar Grupo de Material Didático!", "danger");
				Utility::redirect($cad->arqlis);
			}
			...
			if (Utility::Vazio($msg)) {
				...
				$params = array();
				array_push($params, array('name'=>'gmd_nome',         'value'=>$gmd_nome,        'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'gmd_usu_alteracao','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT,'operador'=>'SET'));
				array_push($params, array('name'=>'gmd_dataalteracao','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'gmd_pre_codigo',   'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));
				array_push($params, array('name'=>'gmd_uuid',         'value'=>$uuid,            'type'=>PDO::PARAM_STR,'operador'=>'WHERE'));
				array_push($params, array('name'=>'gmd_codigo',       'value'=>$id,              'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));

				$sql = Utility::geraSQLUPDATE("gruposmateriaisdidaticos", $params);
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Grupos de Materiais Didáticos<?php echo $textoacao; ?></div>
		<br/>
...
<fieldset style="width:730px;" class="classfieldset1">
   <legend class="classlegend1">Dados do Grupo de Material Didático</legend>
 ...
 <tr>
  <td align="left">
    <label class="classlabel1">Código do Grupo:&nbsp;</label>
  	<input type="text" maxlength="100" name="gmd_codigo" id="gmd_codigo" disabled="disabled" class="classinput1" style="width:150px;text-align:right;background-color:#bcbcbc;" value="<?php echo $gmd_codigo; ?>">
  </td>
 </tr>
 ...
 <tr>
  <td align="left">
    <label class="classlabel1">Nome do Grupo:&nbsp;<label class="labelasterisco">*&nbsp;</label></label>
  	<input type="text" maxlength="50" name="gmd_nome" id="gmd_nome" class="classinput1 inputobrigatorio" style="width:450px" value="<?php echo $gmd_nome; ?>">
  </td>
 </tr>
