<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROENTRADASALMOXARIFADOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Entradas de Almoxarifados");
		Utility::redirect("acessonegado.php");
	}

	$utility->SalvaCadastroAcessado(basename($_SERVER['SCRIPT_FILENAME']), "Cadastro de Entradas de Almoxarifados");
	...
	$campos[1][0] = "eal_codigo";
	$campos[1][1] = "Código da Entrada";
	$campos[2][0] = "eal_numeronota";
	$campos[2][1] = "Número da Nota Fiscal";
	$campos[3][0] = "eal_serienota";
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROENTRADASALMOXARIFADOSEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROENTRADASALMOXARIFADOSEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Entradas de Almoxarifados!", "danger");
		} else {
			$eal_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($eal_codigo, "entradasalmoxarifados")) {
				//$listaalmoxarifados = $utility->getListaEntradaalmoxarifadosAtualizacaoEstoque($eal_codigo);
				...
				$sql = Utility::geraSQLDELETE("entradasalmoxarifados", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				...
				Utility::setMsgPopup("Entrada de Almoxarifados Excluída com Sucesso!", "success");
			}
		}
	}
?>
...
var DialogTelaExcluir = $("#telaexcluir_tela").dialog({
	autoOpen: false,
	height: 250,
	width: 550,
	modal: true,
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirentradaalmoxarifados&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
		...
		"Sair": function() {
			DialogTelaExcluir.dialog("close");
		}
	}
});
...
$(".form_btn_itens").on("click", function(e) {
	...
	$("#aux_eal_codigo").val(codigo);
	$("#aux_eal_uuid").val(uuid);

	var url = 'ajax_itensentradasalmoxarifados.php?id=' + codigo + '&uuid=' + uuid + '&time=' + $.now();
...
$("#linkrel1").on("click", function(e) {
	...
	$.ajax({
	url: "processajax.php?acao=gerarrel1entradaalmoxarifado&arq=" + arq1 + "&time=" + $.now(),
	type: "get",
	dataType: "json",
	success: function(response) {
		sleep(100);
...
$("#linkrel2").on("click", function(e) {
	...
	$.ajax({
	url: "processajax.php?acao=gerarrelsintetico1entradaalmoxarifado&arq=" + arq1 + "&time=" + $.now(),
	type: "get",
	dataType: "json",
	success: function(response) {
		sleep(100);
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Entradas de Almoxarifados</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Entradas de Almoxarifados</legend>
...
<?php
	...
	if ($unidadesocial > 0)
		$unidade = "AND e.eal_uso_codigo = ".$unidadesocial;
	else
		$unidade = "";

	if ($alm_codigo > 0)
		$stralm_codigo = "AND e.eal_codigo IN (SELECT i.iea_eal_codigo FROM itensentradasalmoxarifados i WHERE i.iea_pre_codigo = $CodigoPrefeitura AND i.iea_alm_codigo = $alm_codigo)";
	else
		$stralm_codigo = "";

	$sql = "SELECT e.*, f.for_nome FROM entradasalmoxarifados e LEFT JOIN fornecedores f
	...
	$_SESSION["rel_eal_sql"]        = $sql;
	$_SESSION["rel_eal_dataini"]    = $dataini;
	$_SESSION["rel_eal_datafim"]    = $datafim;
	$_SESSION["rel_eal_uso_codigo"] = $unidadesocial;
...
<tr>
	...
	<td align="center">
		...
		<tr height="10px">
			<td colspan="4">
				<label class="classlabel1">Unidade de Saúde:&nbsp;</label>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("eal_codigo", "Código da Entrada", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("for_nome", "Nome do Fornecedor", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("eal_dataentrada", "Data da Entrada", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("eal_uso_codigo", "Unidade de Saúde", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("eal_dataemissao", "Data da Emissão", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("eal_numeronota", "Número da Nota Fiscal", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("eal_serienota", "Série da Nota Fiscal", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->eal_uuid."&id=".$row->eal_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center" valign="botton">
			...
			codigo="<?php echo $row->eal_codigo; ?>"
			uuid="<?php echo $row->eal_uuid; ?>"
			type="button" style="border: 0; background: transparent"><img src="imagens/view1.png"/>
			</button>
	      </td>

		  <td align="center">
		   <?php echo $row->eal_codigo; ?>
		  </td>
		  <td align="left">
		  ...
		  <td align="center">
		   &nbsp;<?php echo Utility::formataData($row->eal_dataentrada); ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $utility->getNomeCadastro($row->eal_uso_codigo, "unidadessocial"); ?>
		  </td>
		  <td align="center">
		   &nbsp;<?php echo Utility::formataData($row->eal_dataemissao); ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->eal_numeronota; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->eal_serienota; ?>
		  </td>
...
<!-- Tela de Itens -->
<div id="telaitens_tela" title="Itens da Entrada de Almoxarifados">
<input type="hidden" name="aux_eal_codigo" id="aux_eal_codigo"/>
<input type="hidden" name="aux_eal_uuid"   id="aux_eal_uuid"/>
<div id="itens_tela"></div>
</div>
<!-- Tela de Itens -->

<!-- Tela de Relatórios -->
<div id="telarelatorios_tela" title="Relatórios - Entradas de Almoxarifados">
<table width="80%" id="customers" border="1" align="center" style="background-color:#ebf5fe;">
	<tr>
				...
				<td style="border:0px;" align="left">
					<a href="#" class="button_rel1 button_rel1-blue">
						...
						<span style="width:350px" id="linkrel1"
							arq1="<?php echo "rel1entradaalmoxarifado_".$rel_uuid.".pdf"; ?>"
							arq2="<?php echo Utility::getPathDownPDF()."rel1entradaalmoxarifado_".$rel_uuid.".pdf"; ?>">
						Relatório de Entradas de Almoxarifados
						</span>
					</a>
				</td>
			</tr>
				...
				<td style="border:0px;" align="left">
					<a href="#" class="button_rel1 button_rel1-blue">
						...
						<span style="width:350px" id="linkrel2"
							arq1="<?php echo "relsintetico1entradaalmoxarifado__".$rel_uuid.".pdf"; ?>"
							arq2="<?php echo Utility::getPathDownPDF()."relsintetico1entradaalmoxarifado__".$rel_uuid.".pdf"; ?>">Relatório Sintético de Entradas de Almoxarifados</span>
					</a>
				</td>
			</tr>
			</table>
