<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROUNIDADESALMOXARIFADOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Unidades de Almoxarifados");
	...
	$campos[0][0] = "ual_nome";
	$campos[0][1] = "Nome do Unidade de Almoxarifado";

	$cad = new MCLASSGrid();

	$args = "";

	$cad->ordemdefault  = "ual_nome";
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROUNIDADESALMOXARIFADOSEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROUNIDADESALMOXARIFADOSEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Unidade de Almoxarifado!", "danger");
		} else {
			$ual_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($ual_codigo, "unidadesalmoxarifados")) {
				$params = array();
				array_push($params, array('name'=>'ual_pre_codigo','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'ual_codigo',    'value'=>$ual_codigo,      'type'=>PDO::PARAM_INT));
				$sql = Utility::geraSQLDELETE("unidadesalmoxarifados", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				Utility::setMsgPopup("Unidade de Almoxarifado Excluída com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirunidadealmoxarifado&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Unidades de Almoxarifados</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Unidades de Almoxarifados</legend>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("ual_codigo", "Código do Unidade de Almoxarifado", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("ual_nome", "Nome do Unidade de Almoxarifado", $args); ?>
			</th>
			<th width="40px">
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->ual_uuid."&id=".$row->ual_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center">
		   <?php echo $row->ual_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->ual_nome; ?>
		  </td>
		  <td align="left">
		  ...
		  <td align="center" valign="botton">
			...
			codigo="<?php echo $row->ual_codigo; ?>"
			nome="<?php echo $row->ual_nome; ?>"
...
<table width="80%" id="customers" border="1" align="center" style="background-color:#ebf5fe;">
	<tr>
		<td align="right" style="width:250px">
			Código do Unidade de Almoxarifado:&nbsp;
		</td>
		<td align="left">
	...
	<tr>
		<td align="right">
			Nome do Unidade de Almoxarifado:&nbsp;
		</td>
		<td align="left">
