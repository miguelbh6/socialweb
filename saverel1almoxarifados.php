...
class Rel1AlmoxarifadosPDF extends tFPDF {
	...
	private function geraCabecalho() {
		...
		$this->Cell(278,5,'RELATÓRIO ANALÍTICO DE ALMOXARIFADOS - ESTOQUE',0,1,'C');
	...
	function geraCabecalhoAlmoxarifados() {
		...
		$this->SetXY(32, $ini);
		$this->Cell(120,5,'NOME DO ALMOXARIFADO',0,1,'L');
	...
	function geraRelatorio() {
		...
		while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
			...
			$this->SetXY(32, $ini);
			$this->Cell(120,5,substr(utf8_decode($row->alm_nome),0,63),0,1,'L');

			$this->SetXY(153, $ini);
			$this->Cell(30,5,Utility::formataNumero2($row->alm_estoqueminimo),0,1,'R');

			$unidadesocial = $_SESSION["rel_alm_unidadesocial"];
			...
			$vm = $utility->getValorMedioAlmoxarifados($row->alm_codigo);
