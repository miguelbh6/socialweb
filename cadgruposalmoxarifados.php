<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROGRUPOSALMOXARIFADOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Grupos de Almoxarifados");
	...
	$campos[0][0] = "gal_nome";
	$campos[0][1] = "Nome do Grupo de Almoxarifado";
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROGRUPOSALMOXARIFADOSEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROGRUPOSALMOXARIFADOSEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Grupo de Almoxarifado!", "danger");
		} else {
			$gal_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($gal_codigo, "gruposalmoxarifados")) {
				$params = array();
				...
				$sql = Utility::geraSQLDELETE("gruposalmoxarifados", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				Utility::setMsgPopup("Grupo de Almoxarifado Excluído com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirgrupoalmoxarifado&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Grupos de Almoxarifados</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Grupos de Almoxarifados</legend>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("gal_codigo", "Código do Grupo de Almoxarifado", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("gal_nome", "Nome do Grupo de Almoxarifado", $args); ?>
			</th>
...
<table width="80%" id="customers" border="1" align="center" style="background-color:#ebf5fe;">
	<tr>
		<td align="right" style="width:250px">
			Código do Grupo de Almoxarifado:&nbsp;
		</td>
		<td align="left">
