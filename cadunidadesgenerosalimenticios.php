<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROUNIDADESGENEROSALIMENTICIOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Unidades de Gêneros Alimentícios");
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROUNIDADESGENEROSALIMENTICIOSEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROUNIDADESGENEROSALIMENTICIOSEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Unidade de Gênero Alimentício!", "danger");
		} else {
			$uga_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($uga_codigo, "unidadesgenerosalimenticios")) {
				$params = array();
				array_push($params, array('name'=>'uga_pre_codigo','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'uga_codigo',    'value'=>$uga_codigo,      'type'=>PDO::PARAM_INT));
				$sql = Utility::geraSQLDELETE("unidadesgenerosalimenticios", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				Utility::setMsgPopup("Unidade de Gênero Alimentício Excluída com Sucesso!", "success");
			}
		}
	}
?>
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Unidades de Gêneros Alimentícios</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Unidades de Gêneros Alimentícios</legend>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("uga_codigo", "Código da Unidade", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("uga_nome", "Nome da Unidade", $args); ?>
			</th>
			<th width="40px">
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->uga_uuid."&id=".$row->uga_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center">
		   <?php echo $row->uga_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->uga_nome; ?>
		  </td>
		  <td align="left">
		  ...
		  <td align="center" valign="botton">
			...
			codigo="<?php echo $row->uga_codigo; ?>"
			nome="<?php echo $row->uga_nome; ?>"
