<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROENTRADASMATERIAISDIDATICOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Entradas de Materiais Didáticos");
		Utility::redirect("acessonegado.php");
	}

	$utility->SalvaCadastroAcessado(basename($_SERVER['SCRIPT_FILENAME']), "Cadastro de Entradas de Materiais Didáticos");

	if (count($_GET) == 0) {
		unset($_SESSION["mdi_codigo"]);
	...
	$campos[1][0] = "emd_codigo";
	$campos[1][1] = "Código da Entrada";
	$campos[2][0] = "emd_numeronota";
	$campos[2][1] = "Número da Nota Fiscal";
	$campos[3][0] = "emd_serienota";
	...
	if (isset($_POST["mdi_codigo"]))
		$mdi_codigo = $_POST["mdi_codigo"];
	else if (isset($_GET["mdi_codigo"]))
		$mdi_codigo = $_GET["mdi_codigo"];
	else if (isset($_SESSION["mdi_codigo"]))
		$mdi_codigo = $_SESSION["mdi_codigo"];
	else
		$mdi_codigo = 0;

	$mdi_nome = (isset($_POST["mdi_nome"]))? $_POST["mdi_nome"] : "";

	if (Utility::Vazio($mdi_nome)) {
		$mdi_codigo = 0;
	}

	$mdi_nome = $utility->getNomeCadastro($mdi_codigo, "materiaisdidaticos");

	$_SESSION["dataini"]         = $dataini;
	$_SESSION["datafim"]         = $datafim;
	$_SESSION["unidadesocial"] = $unidadesocial;
	$_SESSION["mdi_codigo"]      = $mdi_codigo;
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROENTRADASMATERIAISDIDATICOSEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROENTRADASMATERIAISDIDATICOSEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Entradas de Materiais Didáticos!", "danger");
		} else {
			$emd_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($emd_codigo, "entradasmateriaisdidaticos")) {
				//$listamateriaisdidaticos = $utility->getListaEntradaMateriaisDidaticosAtualizacaoEstoque($emd_codigo);
				...
				$sql = Utility::geraSQLDELETE("entradasmateriaisdidaticos", $params);
				$utility->executeSQL($sql, $params, true, true, true);

				//foreach ($listamateriaisdidaticos as &$mdi_codigo) {
				//	$utility->atualizaEstoqueMaterialdidatico($mdi_codigo);
				//}

				Utility::setMsgPopup("Entrada de Materiais Didáticos Excluída com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirentradamateriaisdidaticos&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
$(".form_btn_itens").on("click", function(e) {
	...
	$("#aux_emd_codigo").val(codigo);
	$("#aux_emd_uuid").val(uuid);

	var url = 'ajax_itensentradasmateriaisdidaticos.php?id=' + codigo + '&uuid=' + uuid + '&time=' + $.now();
...
$("#mdi_nome").autocomplete({
	source: 'processajax.php?acao=getlistamateriaisdidaticos&uso_codigo=0&time=' + $.now(),
	minLength: 5,
	selectFirst: true,
	open: function() {
		$('#ajaxBusy').hide();
...
$("#linkrel1").on("click", function(e) {
	...
	$.ajax({
	url: "processajax.php?acao=gerarrel1entradamaterialdidatico&arq=" + arq1 + "&time=" + $.now(),
	type: "get",
	dataType: "json",
	success: function(response) {
		sleep(100);
...
$("#linkrel2").on("click", function(e) {
	...
	$.ajax({
	url: "processajax.php?acao=gerarrelsintetico1entradamaterialdidatico&arq=" + arq1 + "&time=" + $.now(),
	type: "get",
	dataType: "json",
	success: function(response) {
		sleep(100);
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Entradas de Materiais Didáticos</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Entradas de Materiais Didáticos</legend>
...
<?php
	...
	if ($unidadesocial > 0)
		$unidade = "AND e.emd_uso_codigo = ".$unidadesocial;
	else
		$unidade = "";

	if ($mdi_codigo > 0)
		$strmdi_codigo = "AND e.emd_codigo IN (SELECT i.iem_emd_codigo FROM itensentradasmateriaisdidaticos i WHERE i.iem_pre_codigo = $CodigoPrefeitura AND i.iem_mdi_codigo = $mdi_codigo)";
	else
		$strmdi_codigo = "";

	$sql = "SELECT e.*, f.for_nome FROM entradasmateriaisdidaticos e LEFT JOIN fornecedores f
	...
	$_SESSION["rel_emd_sql"]        = $sql;
	$_SESSION["rel_emd_dataini"]    = $dataini;
	$_SESSION["rel_emd_datafim"]    = $datafim;
	$_SESSION["rel_emd_uso_codigo"] = $unidadesocial;
	$_SESSION["rel_mdi_codigo"]     = $mdi_codigo;
...
<tr>
	...
	<td align="center">
		...
		<tr height="10px">
			<td colspan="4">
				<label class="classlabel1">Nome do Material Didático:&nbsp;</label>
				<input type="hidden" name="mdi_codigo" id="mdi_codigo" value="<?php echo $mdi_codigo; ?>"/>
				<input type="text" maxlength="100" name="mdi_nome" id="mdi_nome" class="classinput1" style="width:450px" value="<?php echo $mdi_nome; ?>">
				<span style="font-size:10px;">Informe o nome do material didático com no mínimo de 5 caracteres.</span>
			</td>
		</tr>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("emd_codigo", "Código da Entrada", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("for_nome", "Nome do Fornecedor", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("emd_dataentrada", "Data da Entrada", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("emd_uso_codigo", "Unidade Social", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("emd_dataemissao", "Data da Emissão", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("emd_numeronota", "Número da Nota Fiscal", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("emd_serienota", "Série da Nota Fiscal", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->emd_uuid."&id=".$row->emd_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center" valign="botton">
			...
			codigo="<?php echo $row->emd_codigo; ?>"
			uuid="<?php echo $row->emd_uuid; ?>"
			type="button" style="border: 0; background: transparent"><img src="imagens/view1.png"/>
			</button>
	      </td>

		  <td align="center">
		   <?php echo $row->emd_codigo; ?>
		  </td>
		  <td align="left">
		  ...
		  <td align="center">
		   &nbsp;<?php echo Utility::formataData($row->emd_dataentrada); ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $utility->getNomeCadastro($row->emd_uso_codigo, "unidadessocial"); ?>
		  </td>
		  <td align="center">
		   &nbsp;<?php echo Utility::formataData($row->emd_dataemissao); ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->emd_numeronota; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->emd_serienota; ?>
		  </td>
...
<!-- Tela de Itens -->
<div id="telaitens_tela" title="Itens da Entrada de Materiais Didáticos">
<input type="hidden" name="aux_emd_codigo" id="aux_emd_codigo"/>
<input type="hidden" name="aux_emd_uuid"   id="aux_emd_uuid"/>
<div id="itens_tela"></div>
</div>
<!-- Tela de Itens -->

<!-- Tela de Relatórios -->
<div id="telarelatorios_tela" title="Relatórios - Entradas de Materiais Didáticos">
<table width="80%" id="customers" border="1" align="center" style="background-color:#ebf5fe;">
	<tr>
				...
				<td style="border:0px;" align="left">
					<a href="#" class="button_rel1 button_rel1-blue">
						...
						<span style="width:350px" id="linkrel1"
							arq1="<?php echo "rel1entradamaterialdidatico_".$rel_uuid.".pdf"; ?>"
							arq2="<?php echo Utility::getPathDownPDF()."rel1entradamaterialdidatico_".$rel_uuid.".pdf"; ?>">
						Relatório de Entradas de Materiais Didáticos
						</span>
					</a>
				</td>
			</tr>
				...
				<td style="border:0px;" align="left">
					<a href="#" class="button_rel1 button_rel1-blue">
						...
						<span style="width:350px" id="linkrel2"
							arq1="<?php echo "relsintetico1entradamaterialdidatico__".$rel_uuid.".pdf"; ?>"
							arq2="<?php echo Utility::getPathDownPDF()."relsintetico1entradamaterialdidatico__".$rel_uuid.".pdf"; ?>">Relatório Sintético de Entradas de Materiais Didáticos</span>
					</a>
				</td>
			</tr>
			</table>
