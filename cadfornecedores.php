<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROFORNECEDORES)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Fornecedores");
		Utility::redirect("acessonegado.php");
	}

	$utility->SalvaCadastroAcessado(basename($_SERVER['SCRIPT_FILENAME']), "Cadastro de Fornecedores");
	...
	$campos[1][0] = "for_cnpjcpf";
	$campos[1][1] = "CPF/CNPJ do Fornecedor";

	$cad = new MCLASSGrid();

	$args = "";

	$cad->ordemdefault  = "for_nome";
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROFORNECEDORESEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROFORNECEDORESEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Fornecedor!", "danger");
		} else {
			$for_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($for_codigo, "fornecedores")) {
				$params = array();
				array_push($params, array('name'=>'for_pre_codigo','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'for_codigo',    'value'=>$for_codigo,      'type'=>PDO::PARAM_INT));
				$sql = Utility::geraSQLDELETE("fornecedores", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				Utility::setMsgPopup("Fornecedor Excluído com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirfornecedor&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Fornecedores</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Fornecedores</legend>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("for_codigo", "Código do Fornecedor", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("for_nome", "Nome do Fornecedor", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("for_endereco", "Endereço do Fornecedor", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->for_uuid."&id=".$row->for_codigo;
