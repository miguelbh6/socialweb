...
class RelSintetico1EntradaGenerosAlimenticiosPDF extends tFPDF {
	...
	private function geraCabecalho() {
		...
		$this->Cell(278,5,'RELATÓRIO SINTÉTICO DE ENTRADAS DE GÊNEROS ALIMENTÍCIOS',0,1,'C');
		...
		$this->SetXY(42, $ini);
		$this->Cell(200,5,'NOME DO GÊNERO ALIMENTÍCIO',0,1,'L');
	...
	function geraRelatorio() {
		...
		if ($_SESSION["rel_gal_codigo"] > 0) {
			$aux = " AND g.gal_codigo = ".$_SESSION["rel_gal_codigo"];
		} else {
			$aux = "";
		}

		$sql = "SELECT g.gal_codigo, g.gal_nome FROM generosalimenticios g WHERE g.gal_pre_codigo = $CodigoPrefeitura $aux ORDER BY g.gal_nome";
		...
		while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
			$entradas = $utility->getTotalEntradasGenerosAlimenticiosDataUnidade($row->gal_codigo, $_SESSION["rel_ega_dataini"], $_SESSION["rel_ega_datafim"], $_SESSION["rel_ega_uso_codigo"]);
			if ($entradas > 0) {
				$ini = $this->GetY() + 1.3;
				$posY = $this->GetY();
				$i++;
				...
				$this->SetXY(10, $ini);
				$this->Cell(30,5,$row->gal_codigo,0,1,'R');

				$this->SetXY(42, $ini);
				$this->Cell(200,5,$row->gal_nome,0,1,'L');
