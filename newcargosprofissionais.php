<?php
	...
	global $PER_CADASTROCARGOSPROFISSIONALSOCIAL;
	if (!$utility->usuarioPermissao($PER_CADASTROCARGOSPROFISSIONALSOCIAL)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Cargo/Função de Profissional Social - 1");
	...
	$cad = new MCLASSGrid();
	$cad->arqlis = "cadcargosprofissionais.php";
	$cad->arqedt = "newcargosprofissionais.php";

	function getDados() {
		foreach($_POST as $key =>$val) {
			if (substr($key, 0, 4) == 'cpr_') {
				global $$key;
	...
	function validaDados() {
		global $msg, $utility, $cpr_nome, $cpr_codigo;
		$msg = "";

		//Nome
		if (Utility::Vazio($cpr_nome)) {
			$msg = "Nome do Cargo/Função Inválido";
		}
		//if ((Utility::Vazio($msg)) && (strlen($cpr_nome) < 10)) {
		//	$msg = "Nome do Cargo/Função Inválido(Poucos caracteres)";
		//}
		if ((Utility::Vazio($msg)) && ($utility->verificaNomeCadastroExiste($cpr_nome, $cpr_codigo, "cargosprofissionais"))) {
			$msg = "Nome do Cargo/Função Já Existe no Cadastro";
		}
		return;
	}
	...
	if ($_GET['acao'] == "inserir") {
		global $PER_CADASTROCARGOSPROFISSIONALSOCIALINSERIR;
		if (!$utility->usuarioPermissao($PER_CADASTROCARGOSPROFISSIONALSOCIALINSERIR)) {
			Utility::setMsgPopup("Você não tem acesso a inserir Cargo/Função de Profissional Social!", "danger");
			Utility::redirect($cad->arqlis);
		}
		...
		//Inserir - Salvar
		if ((isset($_POST['cpr_nome'])) && ($subacao == "salvar")) {
			getDados();
			validaDados();
			...
			if (Utility::Vazio($msg)) {
				...
				$UltimoCodigo  = $utility->getProximoCodigoTabela("cargosprofissionais");
				...
				$params = array();
				array_push($params, array('name'=>'cpr_codigo',      'value'=>$id,              'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'cpr_pre_codigo',  'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'cpr_uuid',        'value'=>$uuid,            'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'cpr_usu_cadastro','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'cpr_datacadastro','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'cpr_nome',        'value'=>$cpr_nome,        'type'=>PDO::PARAM_STR));

				$sql = Utility::geraSQLINSERT("cargosprofissionais", $params);
	...
	if ($_GET['acao'] == "editar") {
		...
		//Carrega Dados
		$sql = "SELECT c.* FROM cargosprofissionais c
		...
		if ($numrows != 1) {
			$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Cargo/Função de Profissional Social - 2");
			Utility::redirect("acessonegado.php");
		}
		...
		//Alterar - Salvar
		if ((isset($_POST['cpr_nome'])) && ($subacao == "salvar")) {
			global $PER_CADASTROCARGOSPROFISSIONALSOCIALALTERAR;
			if (!$utility->usuarioPermissao($PER_CADASTROCARGOSPROFISSIONALSOCIALALTERAR)) {
				Utility::setMsgPopup("Você não tem acesso a alterar Cargo/Função de Profissional Social!", "danger");
				Utility::redirect($cad->arqlis);
			}
			...
			if (Utility::Vazio($msg)) {
				...
				$params = array();
				array_push($params, array('name'=>'cpr_nome',         'value'=>$cpr_nome,        'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'cpr_usu_alteracao','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT,'operador'=>'SET'));
				array_push($params, array('name'=>'cpr_dataalteracao','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'cpr_pre_codigo',   'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));
				array_push($params, array('name'=>'cpr_uuid',         'value'=>$uuid,            'type'=>PDO::PARAM_STR,'operador'=>'WHERE'));
				array_push($params, array('name'=>'cpr_codigo',       'value'=>$id,              'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));

				$sql = Utility::geraSQLUPDATE("cargosprofissionais", $params);
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Cargos/Funções de Profissional Social<?php echo $textoacao; ?></div>
		<br/>
...
<fieldset style="width:730px;" class="classfieldset1">
   <legend class="classlegend1">Dados do Cargo/Função de Profissional Social</legend>
...
<table border="0" width="100%" cellspacing="10" cellpadding="0" align="center">
 <tr>
  <td align="left">
    <label class="classlabel1">Código do Cargo/Função:&nbsp;</label>
  	<input type="text" maxlength="100" name="cpr_codigo" id="cpr_codigo" disabled="disabled" class="classinput1" style="width:150px;text-align:right;background-color:#bcbcbc;" value="<?php echo $cpr_codigo; ?>">
  </td>
 </tr>

 <tr>
  <td align="left">
    <label class="classlabel1">Nome do Cargo/Função:&nbsp;<label class="labelasterisco">*&nbsp;</label></label>
  	<input type="text" maxlength="50" name="cpr_nome" id="cpr_nome" class="classinput1 inputobrigatorio" style="width:450px" value="<?php echo $cpr_nome; ?>">
  </td>
 </tr>
