<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROMATERIAISDIDATICOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Materiais Didáticos");
		Utility::redirect("acessonegado.php");
	}

	$utility->SalvaCadastroAcessado(basename($_SERVER['SCRIPT_FILENAME']), "Cadastro de Materiais Didáticos");

	if (count($_GET) == 0) {
		unset($_SESSION["gmd_codigo"]);
	...
	$campos[0][0] = "mdi_nome";
	$campos[0][1] = "Nome do Material Didático";

	$cad = new MCLASSGrid();

	$args = "";

	$cad->ordemdefault  = "mdi_nome";
	...
	if (isset($_POST["gmd_codigo"]))
		$gmd_codigo = $_POST["gmd_codigo"];
	else if (isset($_GET["gmd_codigo"]))
		$gmd_codigo = $_GET["gmd_codigo"];
	else if (isset($_SESSION["gmd_codigo"]))
		$gmd_codigo = $_SESSION["gmd_codigo"];
	else
		$gmd_codigo = 0;
	...
	$_SESSION["gmd_codigo"]      = $gmd_codigo;
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROMATERIAISDIDATICOSEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROMATERIAISDIDATICOSEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Material Didático!", "danger");
		} else {
			$mdi_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($mdi_codigo, "materiaisdidaticos")) {
				$params = array();
				...
				$sql = Utility::geraSQLDELETE("materiaisdidaticosestoqueunidades", $params);
				...
				$sql = Utility::geraSQLDELETE("materiaisdidaticos", $params);
				$utility->executeSQL($sql, $params, true, true, true);
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirmaterialdidatico&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
$("#linkrel1").on("click", function(e) {
	...
	$.ajax({
	url: "processajax.php?acao=gerarrel1materiaisdidaticos&arq=" + arq1 + "&time=" + $.now(),
	type: "get",
	dataType: "json",
	success: function(response) {
		sleep(100);
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Materiais Didáticos</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Materiais Didáticos</legend>
...
<?php
	...
	if ($tipoestoque == 1) {
		$whereestoque = "AND (GetEstoqueMaterialDidatico($CodigoPrefeitura, $unidadesocial, a.mdi_codigo) > 0)";
	}
	if ($tipoestoque == 2) {
		$whereestoque = "AND (GetEstoqueMaterialDidatico($CodigoPrefeitura, $unidadesocial, a.mdi_codigo) < 0)";
	}
	...
	if ($tipoestoque == 4) {
		$whereestoque = "AND (GetEstoqueMaterialDidatico($CodigoPrefeitura, $unidadesocial, a.mdi_codigo) >= IFNULL(a.mdi_estoqueminimo, 0))";
	}
	if ($tipoestoque == 5) {
		$whereestoque = "AND (GetEstoqueMaterialDidatico($CodigoPrefeitura, $unidadesocial, a.mdi_codigo) < IFNULL(a.mdi_estoqueminimo, 0))";
	}
	...
	if ($gmd_codigo > 0)
		$strgmd_codigo = "AND a.mdi_gmd_codigo = $gmd_codigo";
	else
		$strgmd_codigo = "";
	...
	$sql = "SELECT a.*, g.gmd_nome, u.umd_unidade FROM materiaisdidaticos a
	...
	$_SESSION["rel_mdi_sql"]             = $sql;
	$_SESSION["rel_mdi_unidadesocial"] = $unidadesocial;
	$_SESSION["rel_mdi_tipoestoque"]     = $tipoestoque;
...
<tr>
    ...
    <label class="classlabel1">Grupo de Material Didático:&nbsp;</label>
  	<select name="gmd_codigo" id="gmd_codigo" style="width:300px" class="selectform">
		<?php if (($gmd_codigo == "0") || (Utility::Vazio($gmd_codigo)))
				$aux = "selected='selected'";
			  else
				$aux = "";
		?>
		...
		<?php
			...
			while ($rowGrupo = $objQryGrupo->fetch(PDO::FETCH_OBJ)) {
				if ($gmd_codigo == $rowGrupo->gmd_codigo)
					$aux = "selected='selected'";
			    else
					$aux = "";
				echo "<option value='".$rowGrupo->gmd_codigo."' ".$aux.">".$rowGrupo->gmd_nome."</option>";
            }
		?>
    </select>
  </td>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("mdi_codigo", "Código", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("mdi_ativo", "Ativo", $args); ?>
			</th>
			<th width="40px">
			 	Estoque
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("mdi_nome", "Nome do Material Didático", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("umd_unidade", "Unidade", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("mdi_estoqueminimo", "Estoque Mínimo", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("mdi_controlarlotevalidade", "Controlar Lote", $args); ?>
			</th>
			<th width="40px">
			 	Excluir
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $estoque = $utility->getEstoqueMaterialDidaticoUnidade($unidadesocial, $row->mdi_codigo);

	   if (($row->mdi_estoqueminimo > 0) && ($estoque < $row->mdi_estoqueminimo)) {
			$corlabel = "#ff0000";
	   } else {
			$corlabel = "#000000";
	   }

	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->mdi_uuid."&id=".$row->mdi_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center">
		   <?php echo $row->mdi_codigo; ?>
		  </td>
		  <td align="center">
		   <input id="listativo[]" name="listativo[]" disabled="disabled" type="checkbox" value="1" <?php if ($row->mdi_ativo) echo "checked"; ?> class="estiloradio">
		  </td>
		  <td align="right">
			   <table border="0">
			   <tr>
				...
				<td style="border:0px">
					...
					codigo="<?php echo $row->mdi_codigo; ?>"
		  ...
		  <td align="left">
		   &nbsp;<?php echo $row->mdi_nome; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->umd_unidade; ?>
		  </td>
		  <td align="right">
		   &nbsp;<?php echo Utility::formataNumero2($row->mdi_estoqueminimo); ?>&nbsp;
		  </td>
		  <td align="center">
		   <input id="listcontrolarlotevalidade[]" name="listcontrolarlotevalidade[]" disabled="disabled" type="checkbox" value="1" <?php if ($row->mdi_controlarlotevalidade) echo "checked"; ?> class="estiloradio">
		  </td>

		  <td align="center" valign="botton">
			<button class="form_btn_excluir" name="btnexcluir" id="btnexcluir"
			codigo="<?php echo $row->mdi_codigo; ?>"
			nome="<?php echo $row->mdi_nome; ?>"
...
<!-- Tela de Relatórios -->
<div id="telarelatorios_tela" title="Relatórios - Materiais Didáticos">
<table width="80%" id="customers" border="1" align="center" style="background-color:#ebf5fe;">
	<tr>
				...
				<td style="border:0px;" align="left">
					<a href="#" class="button_rel1 button_rel1-blue">
						...
						<span style="width:280px" id="linkrel1"
							arq1="<?php echo "rel1materiaisdidaticos_".$rel_uuid.".pdf"; ?>"
							arq2="<?php echo Utility::getPathDownPDF()."rel1materiaisdidaticos_".$rel_uuid.".pdf"; ?>">
