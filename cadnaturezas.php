<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTRONATUREZAS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Naturezas");
	...
	$campos[0][0] = "nat_nome";
	$campos[0][1] = "Nome da Natureza";

	$cad = new MCLASSGrid();

	$args = "";

	$cad->ordemdefault  = "nat_nome";
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTRONATUREZASEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTRONATUREZASEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Natureza!", "danger");
		} else {
			$nat_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($nat_codigo, "naturezas")) {
				$params = array();
				array_push($params, array('name'=>'nat_pre_codigo','value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'nat_codigo',    'value'=>$nat_codigo,      'type'=>PDO::PARAM_INT));
				$sql = Utility::geraSQLDELETE("naturezas", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				Utility::setMsgPopup("Natureza Excluída com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirnatureza&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Naturezas</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Naturezas</legend>
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("nat_codigo", "Código da Natureza", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("nat_nome", "Nome da Natureza", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->nat_uuid."&id=".$row->nat_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center">
		   <?php echo $row->nat_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $row->nat_nome; ?>
		  </td>

		  <td align="center" valign="botton">
			<button class="form_btn_excluir" name="btnexcluir" id="btnexcluir"
			codigo="<?php echo $row->nat_codigo; ?>"
			nome="<?php echo $row->nat_nome; ?>"
