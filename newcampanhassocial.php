<?php
	...
	global $PER_CADASTROCAMPANHASSOCIAL;
	if (!$utility->usuarioPermissao($PER_CADASTROCAMPANHASSOCIAL)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Campanhas Social - 1");
	...
	$cad = new MCLASSGrid();
	$cad->arqlis = "cadcampanhassocial.php";
	$cad->arqedt = "newcampanhassocial.php";

	function getDados() {
		foreach($_POST as $key =>$val) {
			if (substr($key, 0, 4) == 'cso_') {
				global $$key;
	...
	function validaDados() {
		global $msg, $utility, $cso_nome, $cso_codigo;
		$msg = "";

		//Nome
		if (Utility::Vazio($cso_nome)) {
			$msg = "Nome da Campanha Social Inválido";
		}
		//if ((Utility::Vazio($msg)) && (strlen($cso_nome) < 10)) {
		//	$msg = "Nome da Campanha Social Inválido(Poucos caracteres)";
		//}
		if ((Utility::Vazio($msg)) && ($utility->verificaNomeCadastroExiste($cso_nome, $cso_codigo, "campanhassocial"))) {
				$msg = "Nome da Campanha Social Já Existe no Cadastro";
		}
		return;
	}
	...
	if ($_GET['acao'] == "inserir") {
		global $PER_CADASTROCAMPANHASSOCIALINSERIR;
		if (!$utility->usuarioPermissao($PER_CADASTROCAMPANHASSOCIALINSERIR)) {
			Utility::setMsgPopup("Você não tem acesso a inserir Campanha Social!", "danger");
			Utility::redirect($cad->arqlis);
		}
		...
		//Inserir - Salvar
		if ((isset($_POST['cso_nome'])) && ($subacao == "salvar")) {
			getDados();
			validaDados();
			...
			if (Utility::Vazio($msg)) {
				...
				$UltimoCodigo  = $utility->getProximoCodigoTabela("campanhassocial");
				...
				$params = array();
				array_push($params, array('name'=>'cso_codigo',      'value'=>$id,              'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'cso_pre_codigo',  'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'cso_uuid',        'value'=>$uuid,            'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'cso_usu_cadastro','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT));
				array_push($params, array('name'=>'cso_datacadastro','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR));
				array_push($params, array('name'=>'cso_nome',        'value'=>$cso_nome,        'type'=>PDO::PARAM_STR));

				$sql = Utility::geraSQLINSERT("campanhassocial", $params);
	...
	if ($_GET['acao'] == "editar") {
		...
		//Carrega Dados
		$sql = "SELECT c.* FROM campanhassocial c
		...
		if ($numrows != 1) {
			$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Novo/Editar de Campanhas Social - 2");
			Utility::redirect("acessonegado.php");
		}
		...
		//Alterar - Salvar
		if ((isset($_POST['cso_nome'])) && ($subacao == "salvar")) {
			global $PER_CADASTROCAMPANHASSOCIALALTERAR;
			if (!$utility->usuarioPermissao($PER_CADASTROCAMPANHASSOCIALALTERAR)) {
				Utility::setMsgPopup("Você não tem acesso a alterar Campanha Social!", "danger");
				Utility::redirect($cad->arqlis);
			}
			...
			if (Utility::Vazio($msg)) {
				...
				$params = array();
				array_push($params, array('name'=>'cso_nome',         'value'=>$cso_nome,        'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'cso_usu_alteracao','value'=>$UsuarioLogado,   'type'=>PDO::PARAM_INT,'operador'=>'SET'));
				array_push($params, array('name'=>'cso_dataalteracao','value'=>$DataHoraHoje,    'type'=>PDO::PARAM_STR,'operador'=>'SET'));
				array_push($params, array('name'=>'cso_pre_codigo',   'value'=>$CodigoPrefeitura,'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));
				array_push($params, array('name'=>'cso_uuid',         'value'=>$uuid,            'type'=>PDO::PARAM_STR,'operador'=>'WHERE'));
				array_push($params, array('name'=>'cso_codigo',       'value'=>$id,              'type'=>PDO::PARAM_INT,'operador'=>'WHERE'));

				$sql = Utility::geraSQLUPDATE("campanhassocial", $params);
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Campanhas Social<?php echo $textoacao; ?></div>
		<br/>
...
<fieldset style="width:730px;" class="classfieldset1">
   <legend class="classlegend1">Dados do Campanha Social</legend>
...
<table border="0" width="100%" cellspacing="10" cellpadding="0" align="center">
 <tr>
  <td align="left">
    <label class="classlabel1">Código da Campanha:&nbsp;</label>
  	<input type="text" maxlength="100" name="cso_codigo" id="cso_codigo" disabled="disabled" class="classinput1" style="width:150px;text-align:right;background-color:#bcbcbc;" value="<?php echo $cso_codigo; ?>">
  </td>
 </tr>

 <tr>
  <td align="left">
    <label class="classlabel1">Nome da Campanha:&nbsp;<label class="labelasterisco">*&nbsp;</label></label>
  	<input type="text" maxlength="50" name="cso_nome" id="cso_nome" class="classinput1 inputobrigatorio" style="width:450px" value="<?php echo $cso_nome; ?>">
  </td>
 </tr>
