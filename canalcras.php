...
//Validação de Login
if ((isset($_POST['uso_codigo'])) && (isset($_POST['usu_login'])) && (isset($_POST['usu_senha'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "validalogin")) {
	global $TLU_LOGINVALIDO;

	Utility::security();

	$uso_codigo = Utility::somenteNumeros(trim($_POST['uso_codigo']));
	$usu_login  = Utility::maiuscula(trim($_POST['usu_login']));
	$usu_senha  = Utility::maiuscula(trim($_POST['usu_senha']));

	$msg = "";
	$sucess = $utility->validaUnidadesLoginSenha($uso_codigo, $usu_login, $usu_senha, $msg, false, true, false);

	if (!$sucess) {
		$utility->gravaLogUsuario($TLU_LOGININVALIDO, "Login: $usu_login - Senha: $usu_senha - Canal do CRAS");
	} else {
		Utility::redirect("main.php");
	}
}
...
<div class="titulosPag">Canal do CRAS</div>
<br/>

<?php if (false) {/*if ($utility->getDadosPrefeituraBD("pre_bloquearcanalcras")) {
			echo $utility->getDadosPrefeituraBD("pre_msgcanalcras");*/

} else { ?>

<div align="left">
<div id="content">
<form name="idform" id="idform" method="post" action="canalcras.php?acao=validalogin">
<fieldset class="classfieldset1" style="width:580px">
   <legend class="classlegend1">Canal do CRAS</legend>
	  ...
	  <tr>
		<td align="right">
		 ...
		 <td align="left" width="65%">
			<select name="uso_codigo" id="uso_codigo" style="width:294px" class="selectform inputobrigatorio">
