...
class RelSintetico1SaidaAlmoxarifadosPDF extends tFPDF {
	...
	private function geraCabecalho() {
		...
		$this->Cell(278,5,'RELATÓRIO SINTÉTICO DE SAÍDAS DE ALMOXARIFADOS',0,1,'C');
	...
	function geraRelatorio() {
		...
		while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
			$saidas = $utility->getTotalSaidasAlmoxarifadosDataUnidade($row->alm_codigo, $_SESSION["rel_sal_dataini"], $_SESSION["rel_sal_datafim"], $_SESSION["rel_sal_uso_codigo"]);
			if ($saidas > 0) {
				$ini = $this->GetY() + 1.3;
				$posY = $this->GetY();
				$i++;
