<?php
	...
	if (!$utility->usuarioPermissao($PER_CADASTROSAIDASMATERIAISDIDATICOS)) {
		$utility->gravaLogUsuario($TLU_ACESSOINVALIDO, "Tela: Listagem de Saídas de Materiais Didáticos");
		Utility::redirect("acessonegado.php");
	}

	$utility->SalvaCadastroAcessado(basename($_SERVER['SCRIPT_FILENAME']), "Cadastro de Saídas de Materiais Didáticos");
	...
	if ((isset($_POST['aux_id'])) && (isset($_GET['acao'])) && ($_GET['acao'] == "excluir")) {
		...
		global $PER_CADASTROSAIDASMATERIAISDIDATICOSEXCLUIR;
		if (!$utility->usuarioPermissao($PER_CADASTROSAIDASMATERIAISDIDATICOSEXCLUIR)) {
			Utility::setMsgPopup("Você não tem acesso a excluir Saídas de Materiais Didáticos!", "danger");
		} else {
			$smd_codigo = $_POST['aux_id'];
			if ($utility->verificaCodigoCadastroExiste($smd_codigo, "saidasmateriaisdidaticos")) {
				//$listamateriaisdidaticos = $utility->getListaSaidaMateriaisDidaticosAtualizacaoEstoque($smd_codigo);
				...
				$sql = Utility::geraSQLDELETE("saidasmateriaisdidaticos", $params);
				$utility->executeSQL($sql, $params, true, true, true);
				...
				Utility::setMsgPopup("Saída de Materiais Didáticos Excluída com Sucesso!", "success");
			}
		}
	}
?>
...
$("#telaexcluir_tela").dialog({
	...
	buttons: {
		"Excluir": function() {
			...
			$.ajax({
			url: "processajax.php?acao=podeexcluirsaidamateriaisdidaticos&id=" + codigo + "&time=" + $.now(),
			type: "get",
			dataType: "json",
...
$(".form_btn_itens").on("click", function(e) {
	...
	$("#aux_smd_codigo").val(codigo);
	$("#aux_smd_uuid").val(uuid);

	var url = 'ajax_itenssaidasmateriaisdidaticos.php?id=' + codigo + '&uuid=' + uuid + '&time=' + $.now();
...
$("#linkrel1").on("click", function(e) {
	...
	$.ajax({
	url: "processajax.php?acao=gerarrel1saidamateriaisdidaticos&arq=" + arq1 + "&time=" + $.now(),
	type: "get",
	dataType: "json",
	success: function(response) {
		sleep(100);
...
$("#linkrel2").on("click", function(e) {
	...
	$.ajax({
	url: "processajax.php?acao=gerarrelsintetico1saidamateriaisdidaticos&arq=" + arq1 + "&time=" + $.now(),
	type: "get",
	dataType: "json",
	success: function(response) {
		sleep(100);
...
<br style="clear:left"/>
    ...
    <div id="coluna22">
      <div class="margemInterna">
		<div class="titulosPag">Cadastro de Saídas de Materiais Didáticos</div>
		<br/>
...
<fieldset style="width:100%;background-color:#FFFFFF;" class="classfieldset1">
   <legend class="classlegend1">Listagem de Saídas de Materiais Didáticos</legend>
...
<?php
	...
	if ($unidadesocial > 0)
		$unidade = "AND s.smd_uso_codigo = ".$unidadesocial;
	else
		$unidade = "";

	if ($mdi_codigo > 0)
		$strmdi_codigo = "AND s.smd_codigo IN (SELECT i.ism_smd_codigo FROM itenssaidasmateriaisdidaticos i WHERE i.ism_pre_codigo = $CodigoPrefeitura AND i.ism_mdi_codigo = $mdi_codigo)";
	else
		$strmdi_codigo = "";
	...
	$_SESSION["rel_smd_sql"]        = $sql;
	$_SESSION["rel_smd_dataini"]    = $dataini;
	$_SESSION["rel_smd_datafim"]    = $datafim;
	$_SESSION["rel_smd_uso_codigo"] = $unidadesocial;
...
<div align="center" id="customers">
		<table id="tblData" width="100%" align="center" border="0" style="border-collapse:collapse;">
			...
			<th width="40px" nowrap>
			 	<?php echo $cad->geraURLTitulo("smd_codigo", "Código da Saída", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("smd_prf_codigo", "Profissional da Saída", $args); ?>
			</th>
			<th width="40px">
			 	<?php echo $cad->geraURLTitulo("smd_datasaida", "Data da Saída", $args); ?>
			</th>
			<th width="120px">
			 	<?php echo $cad->geraURLTitulo("smd_uso_codigo", "Unidade Social", $args); ?>
			</th>
    ...
    <?php
	 ...
	 while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
	   $lkalterar = $cad->arqedt."?acao=editar&uuid=".$row->smd_uuid."&id=".$row->smd_codigo;
	...
	?>
        <tr id="<?php echo $i; ?>" bgcolor="<?php echo $cor; ?>">
		  ...
		  <td align="center" valign="botton">
			...
			codigo="<?php echo $row->smd_codigo; ?>"
			uuid="<?php echo $row->smd_uuid; ?>"
			type="button" style="border: 0; background: transparent"><img src="imagens/view1.png"/>
			</button>
	     </td>

		  <td align="center">
		   <?php echo $row->smd_codigo; ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $utility->getNomeCadastro($row->smd_prf_codigo, "profissionais"); ?>
		  </td>
		  </td>
		  <td align="center">
		   &nbsp;<?php echo Utility::formataData($row->smd_datasaida); ?>
		  </td>
		  <td align="left">
		   &nbsp;<?php echo $utility->getNomeCadastro($row->smd_uso_codigo, "unidadessocial"); ?>
		  </td>

		  <td align="center" valign="botton">
			<button class="form_btn_excluir" name="btnexcluir" id="btnexcluir"
			codigo="<?php echo $row->smd_codigo; ?>"
			nome="<?php echo $utility->getNomeCadastro($row->smd_prf_codigo, "profissionais"); ?>"
...
<!-- Tela de Itens -->
<div id="telaitens_tela" title="Itens da Saída de Materiais Didáticos">
<input type="hidden" name="aux_smd_codigo" id="aux_smd_codigo"/>
<input type="hidden" name="aux_smd_uuid"   id="aux_smd_uuid"/>
<div id="itens_tela"></div>
</div>
<!-- Tela de Itens -->

<!-- Tela de Relatórios -->
<div id="telarelatorios_tela" title="Relatórios - Saídas de Materiais Didáticos">
<table width="80%" id="customers" border="1" align="center" style="background-color:#ebf5fe;">
	<tr>
				...
				<td style="border:0px;" align="left">
					<a href="#" class="button_rel1 button_rel1-blue">
						...
						<span style="width:350px" id="linkrel1"
							arq1="<?php echo "rel1saidamateriaisdidaticos_".$rel_uuid.".pdf"; ?>"
							arq2="<?php echo Utility::getPathDownPDF()."rel1saidamateriaisdidaticos_".$rel_uuid.".pdf"; ?>">
						Relatório de Saídas de Materiais Didáticos
						</span>
					</a>
				</td>
			</tr>
				...
				<td style="border:0px;" align="left">
					<a href="#" class="button_rel1 button_rel1-blue">
						...
						<span style="width:350px" id="linkrel2"
							arq1="<?php echo "relsintetico1saidamateriaisdidaticos_".$rel_uuid.".pdf"; ?>"
							arq2="<?php echo Utility::getPathDownPDF()."relsintetico1saidamateriaisdidaticos_".$rel_uuid.".pdf"; ?>">Relatório Sintético de Saídas de Materiais Didáticos</span>
					</a>
				</td>
			</tr>
			</table>
