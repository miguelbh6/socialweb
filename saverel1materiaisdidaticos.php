...
class Rel1MateriaisDidaticosPDF extends tFPDF {
	...
	private function geraCabecalho() {
		...
		if (!isset($_SESSION["rel_mdi_tipoestoque"])) {
			return;
		}
		...
		$this->Cell(278,5,'RELATÓRIO ANALÍTICO DE MATERIAIS DIDÁTICOS - ESTOQUE',0,1,'C');

		$ini += 5;
		$this->SetXY(10, $ini);
		$this->SetFont('Arial','B',12);
		$this->Cell(278,5,'TIPO DE ESTOQUE: '.utf8_decode(Utility::maiuscula(Utility::getDescTipoEstoque($_SESSION["rel_mdi_tipoestoque"]))),0,1,'C');
	}
	...
	function geraCabecalhoMateriaisDidaticos() {
		...
		$this->SetXY(32, $ini);
		$this->Cell(120,5,'NOME DO MATERIAL DIDATICO',0,1,'L');
	...
	function geraRelatorio() {
		...
		if (!isset($_SESSION["rel_mdi_sql"])) {
			return;
		}

		$sql = $_SESSION["rel_mdi_sql"];
		...
		while ($row = $objQry->fetch(PDO::FETCH_OBJ)) {
			...
			$this->SetXY(10, $ini);
			$this->Cell(20,5,$row->mdi_codigo,0,1,'R');

			$this->SetXY(32, $ini);
			$this->Cell(120,5,substr(utf8_decode($row->mdi_nome),0,63),0,1,'L');

			$this->SetXY(153, $ini);
			$this->Cell(30,5,Utility::formataNumero2($row->mdi_estoqueminimo),0,1,'R');

			$unidadesocial = $_SESSION["rel_mdi_unidadesocial"];
			...
			$vm = $utility->getValorMedioMateriaisDidaticos($row->mdi_codigo);
